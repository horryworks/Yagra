...
> 🚨 **パスワードは URL に使える文字だけにしてください。** 構成はこれを
> `postgres://yagra:<password>@postgres:5432/yagra` に展開しますが、percent-encode は
> できません（文字列テンプレートには手段がありません）。**`/` `@` `:` `?` `#`** を含む
> パスワードは URL をそこで終わらせ、コアは URL を名指しして起動を拒否します。
> 上のスニペットが使う `openssl rand -hex 16` はこれらの文字を 1 つも生成しません。
> **`openssl rand -base64` は生成します** —— base64 の文字集合には `/` が含まれ、
> 24 バイトの値なら高い確率で 1 つ入ります。これらの文字を含むパスワードをどうしても
> 使うなら percent-encode してください（`/` → `%2F`、`@` → `%40`、`:` → `%3A`、
> `?` → `%3F`、`#` → `%23`）。
...
| `YAGRA_RESULT_QUEUE_CAP` | `16384` | VictoriaMetrics が遅くなっている間、メトリクス層が抱えられるポール結果の件数。系列数が多いと VictoriaMetrics は周期的に失速し、この上限を超えたぶんは捨てられる（メトリクスは best-effort 層なので設計どおりだが、グラフの穴にはなる）。**変える前にこのデプロイの `yagra_vm_backlog_needed_high_water` を読むこと** —— 上限が無ければキューがどこまで伸びたかを表す値なので、そのままこの設定を決める数字になる。24 ポートで 1 件約 21 KB なのでメモリは線形に増える。`YAGRA_VM_WRITERS` の本数で割られる。131072 を超える値は丸められる（ログに出る） |
| **API とセキュリティ** | | |
...
| **WebUI からのアップグレード**（構成 **A**。下 6 つは core ではなく `yagra-updater` サイドカーが読みます） | | |
...
| `YAGRA_UPGRADE_MIN_FREE_BYTES` | `3221225472`（3 GiB） | アップグレードが**何かを書き込む前に**要求する空き容量。事前バックアップは PostgreSQL のフルダンプと VictoriaMetrics のスナップショットで、リリース 3 イメージはその隣に展開されます —— どれも同じホスト上です。この確認が無いと、既に満杯のホストに**まずバックアップを書き足してから** pull に失敗することになり、状況を改善するどころか悪化させます。Docker のストレージとデプロイディレクトリのうち**少ないほう**を見ます。どちらも測れないホストは、その旨を記録した上で続行します。`0` で確認を無効化します。バイト数として読めない値（`3g` や打ち間違い）は既定値に戻すので、確認は有効なままです |
| `YAGRA_PULL_POLICY` | `always` | 起動のたびに Compose が 3 つのイメージを再取得するかどうか。既定は従来どおりなので、既存のデプロイは何も変わりません。イメージがレジストリ由来でないホストでは `missing` にします —— サーバ移転は 3 つのイメージを新ホストへ直接読み込むので、その後は元のマシンでしか応答しないレジストリに手を伸ばさずに自力で起動できなければなりません |
| `YAGRA_BACKUP_SKIP_METRICS` | `0` | `1` にすると VictoriaMetrics のスナップショットをバックアップから外し、到達できないストアと同じ扱いで「省いた」ことをマニフェストに記録します。メトリクスのチェックを外して移転したときに設定されます。アップグレード経路がこれを設定することはありません |
| `YAGRA_UPGRADE_KEEP_RELEASES` | `1` | 導入するバージョンの**手前**に何世代残すか —— 対象はこのリポジトリの 3 イメージと、デプロイディレクトリの `yagra-backup-*` だけです。ホスト上の他のものには一切触れず、後始末は新しいバージョンが健全に動いていることを確認した後にのみ走ります。既定が `1` なのは、WebUI の「戻す」が直前のバージョンへの 1 ホップだからです —— そこだけは再ダウンロード無しで成立している必要があります。2 つ以上戻すのは元々手動操作で、再取得を伴って構いません。再取得できない閉域では大きくしてください。`0` は「今のバージョン以外すべて削除」ですが、**バックアップだけは必ず 1 つ残ります** —— 最新のものは、まさにこのアップグレードが取ったものだからです |
...
- **構成への独自の変更は `docker-compose.local.yml` に書きます。`docker-compose.deploy.yml` は編集しないでください。** アップグレードは構成を対象イメージの中から取り出して設置します。おかげで構成とイメージの版がずれることはありませんが、代わりに**そのファイルへの編集はアップグレードのたびに消えます**。隣に置いた 2 枚目のファイルは触られません。WebUI のアップグレードも「リモートポーラーを受け入れる」の切り替えも、そのファイルがあれば `-f` をもう 1 枚渡すので、**そこに書いた内容はアップグレードを越えて残ります**（`.env` が残るのと同じ理由）。Compose は後の `-f` を先の `-f` に重ねるので、上書き側には変えるキーだけ書きます。
  ...
  ⚠️ ポーラーを `network_mode: host` にする場合（イベントとノードの突き合わせに本当の送信元 IP が
  要る環境向けに、構成自身が勧めています）は、上書き側に `ports: !reset []` も要ります。両者は
  排他だからです。`!reset` は **Compose v2.24 以降**（2024-02）。**足すだけ**の上書きなら 2.x
  なら何でも動きます。
