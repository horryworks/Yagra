...
> [!WARNING]
> **v0.2.3 / v0.2.4 / v0.2.5 をお使いの場合は、至急アップグレードしてください。** この 3
> つのリリースは、1 台の機器につき**チェックを 1 本しかポーリングしません**。SNMP ノードでは
> システムスカラーだけが届き、インターフェース walk・ベンダーの機器状態表・トポロジ walk、さらには
> ICMP までが毎周期捨てられます — つまり **SNMP の収集が黙って止まります**。しかも何も知らせません。
> 生き残るチェックが**そのまま死活監視のチェック**なので、ノードは `ok` のまま、欠測も記録されず、
> アラートも上がりません。規模が小さいほど症状は重く、初めて導入した環境ほど強く踏みます。v0.2.6 で
> 修正済みです。詳細は [RELEASE_NOTES.ja.md](RELEASE_NOTES.ja.md) を参照してください。

> ステータス: **v0.3.30 — Meraki MS のスイッチのポートと MR のアクセスポイントを SNMP のスイッチのポートやコントローラ配下の AP と同じように監視する、Meraki MX の WAN 回線・Auto VPN・warm spare の組を見張る、MX を WAN ではなく LAN 側のアドレスでフォルダに振り分ける、Meraki の遅い読み取りが死活の収集を待たせない、値が届かなくなっただけではポートのアラートを閉じない**
...
> 各バイナリは **OpenTelemetry トレース**（オプトイン）を送出し、Yagra ヘルスではコアと各ポーラの
> **ホストリソース傾向**（CPU / ロード / メモリ / ディスク / ネットワーク）を表示、再起動時にはグレースフルに停止
...
> ます（オプトイン）。**AI アシスタントが Yagra を組み込みの MCP ツール面**（`/mcp`、初回起動から有効）から
...
> 設定 ▸ サポートバンドルから**サポートバンドルをダウンロード**できます（ヘルス各セクション・適用済み
> マイグレーション・許可リスト方式の環境変数に加え、**core とポーラーの両方**のローテートログを
> 収録します。リモート拠点のポーラーも、要求に応じて自分のログの一定期間分をバス経由で送ります。
> ノードを 1 つ指定すれば、そのノードの保存済み状態も入ります。書き出す前に、**秘匿値を持っている
> 側で**検査します）。そして **LLM 根本原因説明は推測ではなく調査する**ようになりました —
...
> **ディスカバリの掃引は「操れるもの」になりました**。一覧に出て、戻ってきたときにつなぎ直せます。
> 最初に応答したポーラーではなく選んだ拠点から実行でき、途中で停止でき、ping に応答しない
> アドレスは飛ばします — テスト環境の /24 で 5 分 21 秒かかっていた掃引が、その一部で済みます。
> ダッシュボードのカードは **自分で付けた名前と、自分で選んだ回線を持てる**ようになりました。
> ノードをまたいで最大 6 本のインターフェースを 1 枚のグラフに描き、送信を線の上、受信を下に置きます。
> ベンダーのヘルス表は **行ごとに判定する**ようになりました。メモリプール・CPU・温度センサーの 1 つ 1 つが
> 別のチェックになり、アラートに行の名前が付きます。しきい値ルールでは、効かせる行を名前で選べます。
...
- **Webhook** は core API で受信します: `POST /api/v1/ingest/webhook/<source-id>` +
  ソースごとの Bearer トークン（*イベント ▸ Webhook 送信元* で作成）。
- ルール（*アラート ▸ イベントのアラートルール*）では重大度、自動クローズ TTL、任意のクリアパターン
  （例: link-up が link-down を解決）、発報しきい値（M 秒間に N 回）を設定できます。
  受信イベントは *イベント ▸ イベント* で参照できます。
...
Yagra は**読み取り専用の [MCP](https://modelcontextprotocol.io) ツール面**（ADR-028）を初回起動から公開しており、
...
**初回起動から有効です。設定は要りません。** エンドポイントは **API ポート**の `/mcp` に出ます:
...
MCP は公開ダッシュボードが ON でも **常に認証必須**です。既定で有効でも、次の手順でトークンを発行するまでは
何も見えません。完全に外したい場合は core に `YAGRA_ENABLE_MCP=false` を設定して（deploy compose なら
`.env` に追記、`docker-compose.yml` ならコメントを外す）再起動してください。以後 `/mcp` は未マウント＝404 です。
...
存在理由です。無人利用では、トークンの所有者を**サービスアカウント**（Settings ▸ Users →
...
**動作要件.** **Docker** と **Docker Compose** プラグインが動く x86-64 / ARM64 ホスト。評価用なら
**2 vCPU / 4 GB RAM / 20 GB のディスク**、実運用のフリートなら **4 vCPU / 8 GB RAM / 100 GB の
ディスク**が快適です。compose は任意ストアを含めてすべてのサービスを起動し、その全体のメモリ実測は
1.0〜1.4 GB です。ディスク使用量はノード数ではなくメトリクスの系列数 × 保持期間で決まり、系列を
増やすのはノードではなく収集する*インターフェース*です — コンテナ別の実測値とサイジングの計算式は
...
押す前に、移行先のバージョンのリリースノートを読んでください。**その版について保証を取り下げ、
「新規インストールしてください」と言うことがあります。** そのときはリリースノートの
**破壊的変更**の節にそう書いてあり、新規インストールで何を失うかも書いてあります。

**別サーバーへの移設もワンクリックです。** **Settings ▸ 別サーバーへ移設** が、暗号鍵・アカウント・
アラート履歴まで含めたこの環境まるごとを、SSH で新しい Linux ホストへ複製します。Docker が無ければ
そこに導入します。書庫を作ってダウンロードし、移設先で 1 コマンド実行する経路もあります。詳細は
[DEPLOYMENT.md](DEPLOYMENT.md#relocation)。**その書庫にはこの環境の秘密情報がすべて入るので、
暗号鍵そのものと同じ扱いにしてください。**
