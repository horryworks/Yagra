...
> ステータス: **v0.3.27 — Huawei の無線コントローラ配下の AP が監視対象のノードになる（電波はポートとして、SSID はコントローラの値として記録）、インベントリのツリーを要対応のノードだけに絞れてフォルダをノードの間に置ける、アップグレードで落ちたままの機器のアラートが出し直されない、Discovery が選んだ認証情報をすべて試す**
