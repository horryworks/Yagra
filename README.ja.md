...
> ステータス: **v0.3.29 — Cisco の無線コントローラ（AireOS と 9800）が配下の AP を一覧にして取り込む、Cisco のコントローラが表から外した AP は停止になりコントローラが欠けた台数を数える、AireOS のコントローラにモデルとシリアル番号が出る、コントローラが報告しなくなった AP は最後の「正常」ではなく「不明」になる**
