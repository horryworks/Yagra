...
> ステータス: **v0.3.33 — Discovery に、ネットワークがもう見ている未登録の機器（LLDP/CDP の隣・OSPF/BGP の相手・syslog/トラップの送り手）が並び、プロファイルと認証情報を自動検出できる、「隣接」タブに隣のアドレス・機種 / OS・メーカーが出て、監視していない隣をその場で追加できる、Meraki のスイッチにも 「隣接」タブが出る、開いている画面がほかの人の変更に追従する**
