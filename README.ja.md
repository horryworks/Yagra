...
> ステータス: **v0.3.28 — Cisco Meraki の組織を端から端まで監視できる（機器が自分で取り込まれ、IP 範囲でフォルダに振り分けられ、Dashboard の変更に追従する）、Meraki API が組織に応答しなくなったら 1 つのアラートが出る、Dashboard がオフラインと報告した機器は「正常」ではなく停止になる、無線コントローラ配下の AP がスイッチを入れなくても取り込まれる、フォルダを限定したアカウントは資格情報と Meraki の組織を変更できない**
