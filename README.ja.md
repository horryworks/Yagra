...
> ステータス: **v0.1.12。** ICMP / SNMP v2c+v3 / URL 監視 / Cisco Meraki（読み取り専用 Dashboard
...
> 単位のメトリクスを収集します。ローカルアカウントに加えて**シングルサインオン（OpenID Connect）**で
> サインインでき、**コアは高可用**（複数インスタンスを自動リーダー選出＋フェイルオーバーで運用、
> オプトイン）に構成できます。HA ストアは書き換えではなく設定で対応する段階に留まります。
