...
- **Meraki MX traffic is collected again — per WAN uplink.** The Traffic tier asked the Dashboard's `summary/top/devices/byUsage` for the last hour, which a real Dashboard refuses below eight hours (400), so every organization showed Traffic failing and no device ever had a traffic reading; even at eight hours it answered only the organization's top ten devices, with a total and no sent/received split. It now asks `appliance/uplinks/usage/byNetwork` over the tier's own interval and records each MX WAN uplink's average rate as `meraki_uplink_sent_bps` / `meraki_uplink_recv_bps` (WAN1, WAN2 and cellular, every MX of the watched networks). A Meraki MX node's card shows one line per uplink. `meraki_usage_sent_kb`, `meraki_usage_recv_kb` and `meraki_client_count` are no longer produced — none of them had ever been stored — and switches and access points have no traffic reading for now.
