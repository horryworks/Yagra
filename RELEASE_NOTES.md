...
- **Imported Meraki devices are filed into the folder whose IP range holds their address.** "Import devices" used to put every device under `Organization ▸ Network`, so a deployment whose folders mirror its sites — made by hand, or by the NetBox sync — grew a second, parallel tree. A device whose address falls inside exactly one folder's IP range now goes into that folder, and the longest range wins. A device with no address, with an address no range covers, or with an address two folders claim equally goes under the organization's folder as before, and a network's folder is only created when a device actually lands in it. The dialog has a **File by IP range** box, on by default — turn it off for an organization whose sites reuse one private range — and the result line says how many devices went where, and says separately when overlapping ranges kept a device from being filed. (ADR-164)
  - API: `POST /api/v1/meraki/import` accepts `file_by_prefix` (default `true`), and its 201 now also carries `filed { matched, ambiguous, unmatched, no_address }` and `ranges_configured`. `imported` is unchanged.
