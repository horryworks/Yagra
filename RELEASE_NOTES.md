...
- **A Meraki port or radio metric can carry a port-scoped threshold rule.** The rule editor offers only the metrics that publish one series per port, and it read that flag from the generated catalogue, which marked every Yagra-own check metric node-level. So `meraki_port_in_bps`, `meraki_port_out_bps` and `wlan_radio_non_wifi_util_pct` — the three a Meraki collect stores per port or per radio — could not be given an interface-scope rule, although the alert engine was judging them per interface all along.
