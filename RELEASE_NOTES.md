...
## v0.3.33 — Discovery lists the unregistered devices your network already sees (LLDP/CDP neighbors, OSPF/BGP peers, syslog/trap senders) and detects their profile and credential, the Neighbors tab shows each neighbor's address, model / OS and maker and adds an unmonitored one in place, Meraki switches get the Neighbors tab, open screens follow other people's changes

### New Features

- **Discovery finds devices without a range to scan.** Nodes ▸ Discovery now has two tabs. **Scan a range** is the sweep form as before. **Unregistered devices (N)** lists what the devices you already monitor have seen but Yagra does not monitor: hosts in a router's ARP / IPv6 neighbor cache (as before, when that walk is on), LLDP and CDP neighbors that advertise a management address, OSPF neighbors and BGP peers, and senders of syslog or SNMP traps that no node claimed. The LLDP / CDP, routing and syslog sources need nothing switched on. Each row says where it was seen (one chip per source, then the observer and its port) and carries the name a neighbor or a syslog header gave it, which **Monitor** uses as the node name. The list is read 100 rows at a time: **Load 100 more** reads the next page, and while pages remain the screen says how many are loaded and that the filters search only those. Neighbors whose capabilities are only "phone" or "station" are left out, so the phones behind an access switch do not fill the list. `GET /api/v1/discovered-endpoints` gains `name` and `evidence` (`[{source, via_node, via_ifindex, port, detail}]`) on each row and `summary.unmonitored_total`. A row only a syslog or trap sender vouches for has no observing node, so it is listed only to an account whose scope is unrestricted. Behind NAT a sender's address is the translator's — it is shown as evidence, never imported on its own. A row that only a syslog or trap sender vouches for has no **Detect** or **Monitor**: a sender's address can be forged, and detecting it would send your credentials to whoever forged it, so both endpoints answer `409 sender_only` for it — add such a device with **Add node**. When the list is full, rows a monitored device saw are kept ahead of sender-only ones, so a flood of forged senders cannot push a real device off it. A folder-scoped account sees a row only when its first observer is in one of its folders, and then only the evidence that nodes in its folders reported.
- **Unregistered devices can detect their own profile and credential.** Each row on Nodes ▸ Discovery ▸ Unregistered devices has a **Detect** button. It tries your SNMP credentials on that one device, in the order chosen in a **Credentials to try** field above the table — the same list **Scan a range** uses, so a change on either tab is a change on both, picks a profile from what the device answers. The profile and credential dropdowns appear only once it has answered: filled in and marked when something answered, empty to pick by hand (with **Try again**) when nothing did, with a line saying which credential answered and what the device is. Nothing is imported until you press **Monitor**, which now also records the maker and model the device reported. New endpoint `POST /api/v1/discovered-endpoints/{id}/probe` (`ManageConfig`, HA leader) returns a scan id to read with `GET /api/v1/discovery/scan/{id}`; the import endpoint accepts optional `vendor` and `model`. Both endpoints now answer 404 for a row outside a group-scoped caller's scope, as the list already hid it. The import files into no folder, so it answers `403 out_of_scope` to an account limited to certain folders, which cannot see the root.
...
- **The node detail's Neighbors tab shows each neighbor's address, model / OS and maker.** Two new columns. **Address** is the neighbor's management address, and a badge beside the neighbor's name says what that address is to this deployment: Monitored, Monitored (hidden) for a node in a folder you cannot see, Several nodes, or Not monitored; a **Monitoring** column filters by it. **Model / OS** is the CDP platform with the OS version under it, or the LLDP system description. The neighbor's name links to its node when exactly one node you can see owns that address. When an LLDP chassis ID is a MAC address, the maker the IEEE registered it to appears under it — the maker of the network interface, which may not be who made the device. Click a row to see everything the neighbor sent, unabridged. Addresses are matched the way the network map matches them — a node's own address or any address on its interfaces — never by name or chassis ID. `GET /api/v1/nodes/{id}/neighbors` gains `peers` and `mac_vendors`, and the MCP `get_neighbors` tool returns them too; a node in a folder the caller cannot see is never named or identified.
- **CDP neighbors now report their OS version too.** Pollers walk `cdpCacheVersion` and store it in `remote_sys_desc`, the field LLDP's system description already uses. ⚠️ So the first collection after upgrading adds one entry to the Neighbors change history of every node that has CDP neighbors — the record gained a field; nothing was recabled. It happens once, and not at all for LLDP-only nodes.
- **Meraki switches get the Neighbors tab.** The node detail of a Meraki switch (MS) now lists what each port hears over LLDP and CDP, read from the Meraki Dashboard (`switch/ports/topology/discovery/byDevice`) with the switch ports — as often as Settings ▸ System settings says neighbors are collected (hourly by default), and never while neighbor discovery is off. The rows feed the change history, the network map, Discovery ▸ Unregistered devices and the MCP `get_neighbors` tool the way a walked device's do. The Dashboard keeps a neighbor listed for up to a day after it is unplugged, so a removed cable can take that long to go. LLDP capabilities are left blank: the Dashboard reports roles (S-VLAN component, two-port MAC relay) that do not match what the peers advertise. Access points and MX appliances do not get the tab yet.
- **Add an unmonitored neighbor from the Neighbors tab.** On the node detail's Neighbors tab, a neighbor that is not monitored has a **Set up monitoring** button in the Monitoring column at the end of its row. It opens the row with the same **Detect** → **Monitor** steps as Discovery ▸ Unregistered devices, with its own **Credentials to try** field (it starts from what Scan a range last used; a change there lasts while the page is open). A device something else already manages is sent there instead: an access point a wireless controller you can see reports gets **Add as <controller>'s AP**, which imports it the way the controller's AP list does, and a device a Meraki organization lists links to that organization's page. A neighbor that says it is an access point but that nothing manages can be detected, and **Monitor** appears only once it answers SNMP — an access point a controller manages usually does not, and adding it by hand would leave a second node for it once the controller is added. `GET /api/v1/nodes/{id}/neighbors` gains `discovery_id` and `managed_by` on each peer, and the MCP `get_neighbors` tool returns them too.

### Improvements

- **Clearing a filter no longer loses the node you picked from it.** Narrow the inventory tree, pick a node, then clear the filter: the tree now opens the folders above that node, loads its folder, and scrolls it into the middle of the pane, still selected. Before, the right pane kept showing the node while the tree had no row for it — its folder was closed or not yet loaded, and nothing scrolled — which looked like the selection had been dropped. This happens once, when the last filter goes (search term, state / kind / pool, Pinned only, Folders with nodes only); clearing one of two filters moves nothing. The folders it opens stay open.
...
- **A folder-scoped account can no longer create a node where it cannot see it.** `POST /api/v1/discovery/import` put a row with no folder — none chosen, and no folder's IP range claiming it — at the root of the inventory tree, which an account limited to certain folders cannot see. Such an import is now refused whole with `403 out_of_scope` before anything is written; choose a folder.
...
- **`POST /api/v1/meraki/import` reads only each device's `serial`.** Its name, model, network and address now come from what the organization's last sync recorded, never from the request — a page opened before Meraki renamed a device used to create the node under the old name, and that node never followed a rename again. The other fields are still accepted and ignored. A serial the organization's inventory does not hold is refused with `400 unknown_serial`. An MX whose network's LAN side has not been read yet is no longer imported by hand either (automatic import already waited): it is counted in the answer's new `waiting_lan`, and the device list says `lan_pending` for it. The answer also gains `bound_elsewhere`, the devices skipped because another organization already monitors them. The networks to watch are now stored after the import succeeds, not before.
- **Adding a Meraki organization the API key cannot see is refused** with `400 unknown_org`. It used to be stored under its own id as a name, and every sync of it then failed.
- **`GET` and `PUT /api/v1/meraki/orgs/{id}/networks` answer `404` for an unknown organization**, like every other route under it (they answered `200 []` and `204`), and `PUT` refuses an empty `network_ids` with `400 invalid_request`.
- **A configuration bundle no longer carries the nodes a Meraki organization or a wireless controller owns.** Their binding does not travel, so on the target they became ordinary devices and were pinged and SNMP-polled at their LAN address (`0.0.0.0` for a mesh repeater). Add the same organization on the target and its sync recreates them under the same ids; anything that pointed at them is dropped by the import with its usual note.
...
- **A Meraki access point that is a mesh repeater is badged "Repeater".** An MR with no wired uplink reaches the network over Wi-Fi through another AP, and the Dashboard reports no LAN IP for it — which is why it has no address and no IP range files it. It now wears `Repeater` after its Meraki and Wi-Fi badges in the inventory tree, the folder view, the node header and the move dialog, with the reason on hover. It is read from the device listing the sync already makes, so an AP later cabled in loses the badge at the next sync. The node list and detail (`GET /api/v1/nodes`, `GET /api/v1/nodes/{id}`) and the MCP `list_nodes` / `get_node_status` tools carry `meraki_repeater: true` on such a node.
- **Sort the whole inventory, or one folder, by IP range.** The inventory header has a new ⋯ menu with "Sort every node by IP range…", and a folder's right-click menu has "Sort this folder by IP range…". Both open the same preview as "Move by IP range…", but the server collects the nodes, so folders that were never opened count too. One press moves at most 1,000 nodes; when more match, the dialog offers "Continue" for the rest. Behind it is `POST /api/v1/nodes/move-preview/subtree` (`{ group_id }`, or `null` for everything), which returns the first 1,000 proposals, the true totals, and each named node's name and address.

### Improvements

- **The inventory tree's filters are behind one button.** The row of Filter, Pinned only, With nodes and Needs attention under the inventory header is gone, and so is the State / Kind / Pool row Filter opened. A funnel beside the search box opens all of them in one panel: Pinned only, Needs attention and Hide empty folders (the switch formerly called "With nodes") at the top, then State, Kind and Pool as check boxes. The funnel shows how many are in force, and each one is listed as a chip under the header with a ✕ to remove it, beside "Clear all filters". Nothing about how they are kept changed — Pinned only and Hide empty folders still follow the account, the rest is still in the URL, and the header's "N need attention" still presses Needs attention in one click.
- **Discovery sweeps up to a /20 (4,096 addresses) in one go**, where the limit was 1,024 (a /22). A site folder's /20 can now be ticked and swept as one scan, and the address field accepts one too. Behind the screen core sends a sweep that wide to the poller as up to four jobs of at most 1,024 addresses each and shows them as one scan — progress, found devices and "Stop" all cover the whole range — so a busy /20 cannot outgrow a single bus message and leave the scan stuck at "running". The import and its preview accept up to 4,096 devices to match. A sweep still found running after four hours (was two) is dropped from the list.
...
- **An IP-range move no longer pulls a node out of the folder it was filed in.** A node already in the folder its range names, or in any folder beneath it, is now left where it is and counted as "already in the right folder" — it used to be proposed for a move back up into the folder carrying the range, undoing a hand-made placement in a subfolder. `POST /api/v1/nodes/move-preview` returns those ids in a new `in_place` list and no longer lists them under `matched`, `ambiguous` or `unmatched`.
...
- **A folder a NetBox sync creates or moves is visible to a folder-scoped account at once.** The sync wrote folders without telling the rest of core that the tree had changed, so a site NetBox added stayed out of a scoped account's view — and out of the alert configuration — until some unrelated change was saved. A sync of an unchanged NetBox now writes no folder at all.
- **Resuming a paused Meraki organization no longer raises its "API not answering" alert at once.** The failures counted before the pause were kept, and the first check after the resume raised the alert again before a single collect had been sent. A pause, or switching Meraki polling off, now forgets them.
- **A Meraki organization's "API not answering" alert closes when nothing is asked of it any more.** An organization whose networks were all un-watched, or whose devices were all deleted, kept the alert open for good, and its row went on showing a failing collect for a tier that was switched off or had no device left.
- **A Meraki collect that keeps failing in part is counted as such.** When one read of a tier failed every time while the others answered — the Auto VPN read of the uplink tier, say — the organization's page showed "failing once, since the latest collect" for as long as it went on. The poller now sends its report first, so each failure adds to the count.
- **A Meraki collect that cannot be sent is counted as failing.** A collect larger than the message bus accepts (an organization of roughly 13,000 imported devices) failed to publish on every tick with nothing but a log line, and every node kept its last state. It now counts towards the organization's alert.
- **A Meraki sync now uses the organization's settings as they are when it imports**, not as they were when it started. A whole-organization read runs for minutes; switching automatic import off, lowering the cap or turning off filing by range while it ran was ignored.
- **Pausing a Meraki organization, deleting it, or switching Meraki polling off stops a sync already running for it**, within fifteen seconds. A whole-organization read used to go on asking the Dashboard for up to half an hour. A "Sync now" waiting when an organization is paused still runs on resume.
- **"Reading…" no longer stays on a Meraki organization after a sync that failed to clear it**, which also hid "Sync now" until core restarted. Every sync now clears it.
- **One MX network whose LAN read always fails no longer turns every Meraki sync into a whole-organization read.** It is still asked again on each sync.
- **The Meraki topology map is re-derived after a sync moves a node's address** even when a later step of that sync fails.
- **A Meraki collect no longer waits out a rate limit past its time budget.** Availability, uplink and traffic had no budget at all, and a long `Retry-After` could hold any collect past core's five-minute lease, which then sent a second collect beside it. Every collect now stops at four minutes, and a wait that would pass that is not waited.
- **An MX uplink measured against two destinations reports one loss and one latency** — the worst of the two — instead of two conflicting samples of the same series.
- **A Meraki availability row with no status no longer marks the device down.** It gives no reading.
- **An MX network whose single-LAN answer carries no `applianceIp` is read again** rather than remembered for a day as having no LAN side.
- **Emptying a Meraki interval or rate box no longer sends 0.** The dialog shows the accepted range under the box and holds **Save** until every box is in range.
- **The `invalid_subject` error names the `meraki_org:<uuid>` form** the alert subject parser has accepted since the organization alert shipped.
...
- **A folder now lists the subnets its devices carry that its IP prefixes do not cover.** Open a folder in Nodes: below its IP prefixes, "Subnets missing from the IP prefixes" compares every address the devices in that folder and its subfolders report with those folders' IP prefixes, and says for each subnet why it is listed: in no prefix at all, only partly covered, in another folder's prefix (a prefix attached to the wrong folder, or one private range reused at two sites), or only in a parent folder's prefix. It also says how many of the folder's devices it could read addresses from, since a device without an SNMP address walk contributes nothing. A folder holding more than 2,000 devices is not compared; open a folder further down. The same answer is `GET /api/v1/node-groups/{id}/prefix-gaps` and the MCP tool `get_prefix_gaps`; a group-scoped caller is told that another folder holds a subnet, but not which folder or prefix.

### Improvements

- **The inventory tree draws its branches.** Every row now has lines back to its parent folder: a `├` while more rows follow in the same folder and a `└` on the last one, so where a folder ends shows without lining up indentation. The row open in the detail pane lights its branch up to the top folder. Nodes picked with Ctrl or Shift light theirs more faintly, and branches from different folders join where they share a parent. Each folder with picked nodes inside says how many ("3 selected"), including when it is closed. Scrolling into a long folder keeps up to three of its parent folders pinned at the top of the tree; click one to go back to it. The pinned folders hide while you drag.
...
- **A newly added Meraki organization may import up to 10,000 devices by default, instead of 1,000.** An organization of a few thousand devices used to stop at 1,000 on its first sync until someone raised the cap. Organizations already added keep the cap they have. The cap only limits which devices become nodes: every device is still read, and each one's address and folder are decided with the whole organization in view, so devices the cap holds back are filed the same way when they go in later.
- **A newly added Meraki organization collects its MX uplinks' traffic every five minutes by default, instead of every thirty.** The WAN traffic chart used to move in half-hour steps until someone shortened the organization's traffic interval. The read is one request for the whole organization, so the shorter interval costs the Dashboard almost nothing. Organizations already added keep the interval they have — set it to 300 seconds on the organization's page to get the same.
...
- **Meraki MR access points: clients, SSIDs broadcast and radio readings, under the names a controller's access points use** (ADR-168). A new collect tier, `wireless`, reads every access point of an organization from the Dashboard — its clients online (`wireless/clients/overview/byDevice`) and each radio's channel utilization over the last five minutes (`wireless/devices/channelUtilization/byDevice`) — and, every twenty minutes, the SSIDs it broadcasts and each radio's channel and transmit power (`wireless/ssids/statuses/byDevice`). They are published as `wlan_ap_client_count`, `wlan_radio_channel_util_pct`, `wlan_radio_channel` and `wlan_radio_tx_power_dbm` — the names a Cisco or Huawei controller's access points use, so the three vendors compare side by side — plus two new ones, `wlan_ap_ssid_count` and `wlan_radio_non_wifi_util_pct` (the part of the utilization that is not Wi-Fi). An MR stays a Meraki node — its liveness is still `meraki_device_up` and it is not in the wireless AP list — and now wears the access point's badge — a Wi-Fi mark, black on white — beside **Meraki** in the inventory tree, a folder's member list, the node header and the move dialog; `NodeSummary` (`GET /api/v1/nodes`, the folder lists) and MCP `list_nodes` carry a new optional `meraki_product_type` for it. Its radios are rows on an Interfaces tab (2.4 GHz, 5 GHz), and the Overview's Meraki card shows connected clients, SSIDs broadcast and each band's utilization with its non-Wi-Fi share. The tier starts on for **every organization, existing ones included** (migration 0131 adds `wireless_secs`, 300–600 s, default 300, and appends `wireless` to each organization's `enabled_tiers`); `PUT /api/v1/meraki/orgs/{id}/cadence` takes an optional `wireless_secs` and leaves the stored interval alone when it is left out, and `MerakiOrgView` carries it. It is sent only to a poller pool whose every live poller advertises `meraki-wireless` — **upgrade the Meraki pool's pollers** to start collecting. ⚠️ Nothing the Dashboard answers says whether a radio is on, so an MR's radios have no up/down state; and because the Dashboard keeps answering a stopped access point's last SSID configuration as broadcasting, an access point whose radios are not being measured gets no SSID count, channel or power at all rather than stale ones. The SSID read is slow (78 s for 1,710 access points on a real organization) and is made once every twenty minutes in the organization's slow collect lane, so its availability collect does not wait for it (see below); the other reads take a few seconds. ⚠️ Rolling core back a version leaves `wireless` in each organization's tiers, and that core's cadence dialog then fails to save with `400 invalid_tier`; nothing stops collecting. Confirmed against a recorded Dashboard in the lab; not yet against a live organization.
- **Meraki MS switch ports: status, speed and traffic, on the same screens as an SNMP switch's** (ADR-167). A new collect tier, `switch_ports`, reads every switch port of an organization from the Dashboard (`switch/ports/statuses/bySwitch` and one five-minute bucket of `switch/ports/usage/history/byDevice/byInterval`) and publishes it under the names an SNMP switch uses — `if_oper_status`, `if_admin_status`, `if_high_speed` — plus two new per-port gauges, `meraki_port_in_bps` and `meraki_port_out_bps`. A Meraki switch now has an Interfaces tab with each port's state, speed, duplex and traffic and the port charts; its ports rank beside SNMP ports in the busiest-interfaces widget, the interface top-N and deltas, the fleet throughput chart and utilization alerts; and MCP `get_interface_series` / `top_interfaces` read them too. The ports' names in the Dashboard become their aliases, read once an hour. The tier starts on for **every organization, existing ones included** (migration 0130 adds `switch_ports_secs`, 300–600 s, default 300, and appends `switch_ports` to each organization's `enabled_tiers`); `PUT /api/v1/meraki/orgs/{id}/cadence` takes an optional `switch_ports_secs` and leaves the stored interval alone when it is left out, and `MerakiOrgView` carries it. ⚠️ Traffic is a five-minute average read once the Dashboard has filled the bucket, so it is drawn **12–17 minutes behind** the port; a switch that is down or dormant reports no port state at all (every port reads Disconnected there), so it raises its own node-down alert and no port-down per port; error and discard counts are not collected, and the dock hides their chart for a Meraki port. The tier is sent only to a poller pool whose every live poller advertises `meraki-switch-ports`, so an organization is not held up by an older poller — **upgrade the Meraki pool's pollers** to start collecting. It adds two organization-wide reads every five minutes and a third once an hour; measured on a real organization of 854 switches, about 100 s of requests (190 s in the hour the names are read), made in the organization's slow collect lane so that its availability collect does not wait for it (see below). ⚠️ Rolling core back a version leaves `switch_ports` in each organization's tiers, and that core's cadence dialog then fails to save with `400 invalid_tier` until it is upgraded again; nothing stops collecting. Confirmed against a recorded Dashboard in the lab; not yet against a live organization.
- **Meraki MX warm-spare pairs: which MX is primary, and whether a site is running on its spare.** The five-minute inventory sync now also reads each MX's warm-spare role (`appliance/uplink/statuses`) and keeps it with the device (migration 0129 adds one column). An MX's node card shows its pair — its own role, the other MX (linked to its node), and what the pair is doing: normal, running on spare, spare down, both down, or unknown. That state is worked out from the two devices' own availability, because the role the Dashboard reports is the configured one and does not change when the spare takes over. The organization's device list shows the role under the model; the API carries it as `ha_role` on `GET /api/v1/meraki/orgs/{id}/devices` and as `meraki_pair` on `GET /api/v1/nodes/{node_id}`, and MCP's `get_node_status` includes it. A partner in a folder the caller cannot see is not named, and the state then reads unknown. No new alert: a primary that stops raises its own availability alert, as before. A failed role read never fails the sync.
- **Meraki MX Auto VPN reachability, with one alert that escalates.** The uplink collect now also reads `appliance/vpn/statuses`, and each MX gets `meraki_vpn_hubs_reachable`, `meraki_vpn_hubs_unreachable` and `meraki_vpn_hubs_unreachable_pct` (a hub also gets `meraki_vpn_spokes_unreachable`). A default rule on the built-in "Cisco Meraki MX (API)" profile raises a warning when a site loses one of its hubs and escalates the same alert to critical when it reaches none. A hub that is itself down is not counted against its spokes — it raises its own alert — and an MX that is down reports nothing, so its stale row cannot page. The node's card shows "1 of 2 hubs reachable". ⚠️ A site running on its warm spare has no VPN reading: the Dashboard always reports the VPN under the pair's primary.
...
- **A Meraki MX's LAN side is re-read a few networks at a time, and never while availability waits** (ADR-164 increment 16). Each network's VLANs are re-read once a day. The first round reads about sixty networks per sync, so a day later those sixty all went stale together, and for about half an hour every day each sync spent its whole minute re-reading them — holding the lane the switch-port collects run in, whose charts can dot when one runs late. A sync now re-reads at most its share of the day's work (the MX networks × the sync interval ÷ 24 hours, rounded up, plus one — 3 per sync for 350 networks every five minutes), so the clump spreads out over the first day and stays spread. Networks never read are still read as fast as before, since their MX waits for that read to be imported. And "Sync now", when it has to take the fast lane because the slow one is busy, reads no LAN side at all: the fast lane is where availability is collected, and a minute of LAN reads there held availability back a minute. Such a sync chooses from the addresses already known; the next periodic sync reads the rest.
- **The fleet throughput chart asks VictoriaMetrics for its total in a form it can add up as it reads** (ADR-167). The total is now `(sum(counters) + sum(Meraki ports)) or sum(counters) or sum(Meraki ports)` instead of `sum(counters or Meraki ports)`. The numbers are the same, and a stretch with no data is still a gap rather than a 0. Summing each half straight over its series is the form VictoriaMetrics aggregates incrementally; the old form built every interface's series first. How much this saves on a large fleet has not been measured.
- **The NetBox folder badge is NetBox blue on white.** A folder the NetBox integration keeps is marked **NetBox** in the inventory tree; that badge now wears NetBox's own blue on white, as the Meraki one wears Meraki's green, instead of Yagra's accent.
- **An access point's badge is a Wi-Fi mark.** The badge that said **AP** after an access point's name — a node imported from its wireless controller, and a Meraki MR beside its **Meraki** badge — is now a black Wi-Fi mark on white, in the inventory tree, a folder's member list, the node header and the move dialog. Hovering it still says "Access point" (or the kind's name), and a screen reader reads the same.
- **A Meraki organization's slow reads no longer hold up its availability collects.** An organization's collects used to run one at a time, so a switch-port collect (about 140 s, 220 s when it also reads the ports' names) or the access points' SSID read (about 80 s) kept availability, uplink and wireless waiting behind it. Measured on a lab deployment with availability every 60 s: 185 collects in six hours instead of 360, gaps of up to 330 s — a device that went down was noticed that much later — and an access point's client count arriving 390 s apart instead of 300, which its chart drew as a dotted line. The collects now run in two lanes per organization: a fast one for availability, uplink, traffic and the wireless clients and utilization, and a slow one for the switch ports, the SSID read and the periodic inventory sync. The SSID read is now a collect of its own that reports no client count or utilization, so it never adds a sample between two wireless collects; it starts only where the next switch-port collect is at least 120 s away, so the port traffic charts are not interrupted either. Measured on the same lab deployment for an hour afterwards: every availability and uplink interval 60 s, every access point client-count and utilization interval 300 s. An organization is still asked by at most two sessions at once, and **its request rate setting stays the organization's total** — each lane paces at half of it (below 0.2 req/s each lane stops at 0.1, so the two together send up to 0.2 req/s whatever the setting). The hourly request count rises to what the intervals always implied, since collects that used to be skipped for lack of a turn now run (1,145 → 1,349 an hour on the lab organization). "Sync now" takes whichever lane is free and is refused (409) only while both are busy. ⚠️ The collect job gained one field (`ssid_only`); a poller from before it runs the SSID read as a whole wireless collect — correct values, plus one extra client-count and utilization sample every twenty minutes, which at a 600 s wireless interval can dot those charts until the poller is upgraded. No migration or screen changes.
- **A Meraki access point's card charts its history.** Under the tiles, two charts over the selected range (the same 1h–7d control an MX card has): connected clients and SSIDs broadcast on one, and each radio's channel utilization with its non-Wi-Fi share on the other — counts and percentages never share an axis. The charts read back what each collect already stored; they ask Meraki nothing. They keep drawing while the access point is offline, because every point carries its own time — unlike the tiles above, which hide a stopped access point's last SSID count and utilization.
- **The Meraki badges are Meraki green.** The badge beside a Meraki node's name (in its header, in a folder's member list and in the inventory tree) and the badge on a folder a Meraki organization keeps are now drawn in Meraki's green with white text, like its logo. Other badges keep Yagra's accent colour.
...
- **A port's utilisation or traffic alert is no longer closed because its readings stopped** (ADR-076 increment 8). When a tracked port dropped out of the ports above the rules' floor, the evaluator observed it at 0 bits/s — true of a port that went quiet, and false of one whose readings stopped: an SNMP agent that died while the device still answers ping, a walk cut short, a Meraki switch window the Dashboard did not fill. Those alerts closed as recoveries and sent a recovery notification. The evaluator now reads such a port again and observes the value it actually has, so a quiet port recovers through the rule's dwell as before, with its real value in the history; a port with no value at all is held — nothing observed, its alert left open, because an open alert is the only thing on screen saying the data stopped. This applies to SNMP ports as well as Meraki switch ports. An alert on an interface that was removed now stays open until its rule or its node is deleted. The new gauge `yagra_interface_util_held{metric}` counts the port checks held open for want of a value. A port check whose recovery has committed is no longer tracked, so the extra read covers only ports that are still recovering.
- **The API documentation says what it does.** The `target_rps` field (`GET`/`PUT` of a Meraki organization) said the inventory sync runs in the fast lane — the periodic sync runs in the slow one, and "Sync now" takes whichever is free — and that a setting below 0.2 could be exceeded by up to 0.1, where the two lanes then send up to 0.2 req/s in total. `POST /api/v1/meraki/orgs/{id}/sync` named three reads; it also reads the MX uplink statuses and, in the slow lane, the VLANs of the MX networks due to be read, and answers 409 only while both collect lanes are held. `GET /api/v1/nodes/{node_id}/interfaces/{ifindex}/series` and MCP `get_interface_series` now say that a Meraki switch port's `in_bps`/`out_bps` are the Dashboard's stored five-minute averages, 12–17 minutes behind, that its packet, error and discard arrays are always empty (not collected, not "no errors"), and that its `ifindex` is the number Yagra gives the port. `lan_ip` and `ha_role` on a Meraki device now say when each is empty or absent.
- **A Meraki MX is addressed by its LAN, not its WAN, so it is filed into the folder whose IP range holds its site** (ADR-164 increment 15). An MX reports no LAN address to the Dashboard — none of 686 did on a real organization — so it took its WAN1 address: in nobody's IP ranges, often dynamic, and behind an ISP router the same common `192.168.x` as other sites, which filed some MX into another site's folder. An MX's address is now one of its own VLAN IPs: an IP another network of the organization also uses is skipped, and of the rest it takes the lowest-numbered VLAN that lies inside a folder's IP range, else the lowest-numbered VLAN; in a network with VLANs turned off, its single LAN's address. Its WAN address is never used. (Why not simply the lowest VLAN: in 29 networks of 342 on that organization it was a default or guest subnet shared by up to eight sites, and the site's own VLAN came next. Why skip the reused IPs first: on a lab deployment a reused guest subnet happened to lie in a home folder's range, and 196 MX in about a hundred networks all took that one address; skipping them gives 339 networks of 350 a corporate address on the recorded organization, and no two networks the same one.) The inventory sync reads each MX network's VLANs one network at a time — the organization-wide listing is a beta endpoint and answered 404 — for up to 60 s per sync, never-read networks first and then each once a day, so an organization of 350 MX networks is read in about six syncs (half an hour) after the upgrade; migration 0132 adds two columns to `meraki_org_networks` to keep what was read. Until its network has been read an MX has no address in the organization's device list (`lan_ip` is `null` in `GET /api/v1/meraki/orgs/{id}/devices`) and is not imported automatically — it keeps its place under the organization's import cap meanwhile, so the switches and access points behind it cannot take its slot — so it is never filed by an address it does not have yet. **Existing MX nodes take the new address but stay in their folder** — select them in Nodes and use Move by IP range. A failed read keeps what was read before. A warm-spare pair shares its network's VLANs, so both MX now stand at one address, and Nodes ▸ Duplicates no longer lists two nodes bound to different Meraki devices as duplicates for sharing an address. ⚠️ Syslog, traps and flows sent from that shared address are attributed to whichever of the pair's nodes is found first. The VLAN answers were measured on a real organization, and the sync has run against a recording of them in the lab; not yet against a live organization.
- **A port-scoped threshold rule can name a Meraki port or radio metric.** Editing such a rule offers only the metrics that publish one series per port, and that list read a flag from the generated catalogue which marked every Yagra-own check metric node-level. So `meraki_port_in_bps`, `meraki_port_out_bps` and `wlan_radio_non_wifi_util_pct` — the three a Meraki collect stores per port or per radio — were missing from it, although the alert engine was judging them per interface all along. ⚠️ Creating a port rule from a port’s own dialog still offers its fixed list of subjects (traffic, link state, optical power); these three are reachable when a port-scoped rule is edited, or through the API.
- **An interface with no traffic reading says "down" only when it is down.** The two throughput cells printed "down" whenever there was no rate to draw — true of a port the device reports down, and false of one whose state nobody has an answer for. A Meraki access point's radio never reports an operational status (nothing the Dashboard answers says whether a radio is on), so every radio read "down" beside its channel and utilization; a port whose walk has not answered yet was in the same position. Such a row now shows "—".
- **Meraki MX traffic is collected again — per WAN uplink.** The Traffic tier asked the Dashboard's `summary/top/devices/byUsage` for the last hour, which a real Dashboard refuses below eight hours (400), so every organization showed Traffic failing and no device ever had a traffic reading; even at eight hours it answered only the organization's top ten devices, with a total and no sent/received split. It now asks `appliance/uplinks/usage/byNetwork` over the tier's own interval and records each MX WAN uplink's average rate as `meraki_uplink_sent_bps` / `meraki_uplink_recv_bps` (WAN1, WAN2 and cellular, every MX of the watched networks). A Meraki MX node's card shows one line per uplink. `meraki_usage_sent_kb`, `meraki_usage_recv_kb` and `meraki_client_count` are no longer produced — none of them had ever been stored. A switch's traffic is its ports' (see the switch-port feature above); an access point has no traffic reading.
- **A folder with more members than the screen is tall now scrolls down to its last member.** Selecting a folder under All nodes cut its Members list off at the bottom of the pane with no scrollbar, so a large folder — an imported Meraki organization with over a hundred networks, for one — could not be read to the end. The folder's body now scrolls under its header, as a node's tabs do.
...
- **A Meraki organization with more than about two hundred networks is collected again** (ADR-164 increment 12). A collect put every watched network into one URL as `networkIds[]`, and the Dashboard refuses a request line past about 8 KB with `414 Request-URI Too Large` — measured against a real organization. So every availability, uplink and traffic collect of such an organization failed: its devices stayed `unknown` and the organization's collect alert opened. The poller now asks for the whole organization and keeps the watched networks' rows itself, so the request no longer grows with the number of networks. Two of the four listings ignored `networkIds[]` anyway. Watching a small part of a large organization now reads the whole organization's rows (still paged at 1,000). **Upgrade the pollers** — the fix is in the poller; core and the bus are unchanged, and an older poller keeps failing the same way.
...
- **An account restricted to folders can no longer change a Cisco Meraki organization.** Every Meraki write — `POST /api/v1/meraki/orgs`, `POST …/orgs/discover`, `DELETE …/orgs/{id}`, `PUT …/orgs/{id}/enabled`, `…/cadence`, `…/networks` and `…/import-settings`, `POST …/orgs/{id}/sync`, `POST /api/v1/meraki/import` and `PUT /api/v1/meraki/polling` — now answers `403 scope_unsupported` to a caller whose account is limited to folders. The reads are unchanged. An unrestricted Operator or Admin is unaffected. See **Security** below for why.

- **`POST /api/v1/meraki/orgs/{id}/enumerate` is removed, and so is the "Import devices" dialog that called it.** It asked the Dashboard API for an organization's networks and devices on every press, and accepted a partial answer when a page failed. The inventory sync reads the same listing every five minutes and refuses a partial one, so there were two readers of one list and they could disagree. Read `GET /api/v1/meraki/orgs/{id}/devices` instead — it answers from what the last successful sync recorded — and use `POST /api/v1/meraki/orgs/{id}/sync` to refresh it on demand. `POST /api/v1/meraki/import` is unchanged except that an absent `file_by_prefix` now means the organization's own setting rather than `true`. (ADR-164)

- **A Meraki organization that watches no network is no longer collected.** Collection narrows its Dashboard API calls to the watched networks, but an organization with *none* watched was sent an unnarrowed request and collected whole — while Settings ▸ Integrations has always said "Only monitored networks are collected". It now gets no collect at all, and neither does a round in which the watched networks could not be read (that used to widen the collect to every network as well). An organization imported through the old "Import devices" dialog is not affected: that dialog watched the network of every device it imported. One whose networks were all un-checked by hand stops being collected after the upgrade; its row says **N not collected**, and its page offers **Watch these networks**. (ADR-164)

- **`PUT /api/v1/meraki/orgs/{id}/cadence` refuses `enabled_tiers` without `availability`** (`400 availability_required`). Availability is the only tier that says whether a Meraki device is up — uplink and traffic record readings and decide nothing — so an organization saved without it could never report a device down: its nodes sat in `unknown`. The cadence dialog no longer offers that checkbox and says why. On upgrade, an organization stored without the tier gets it back, which adds one paged Dashboard API request per availability interval. (ADR-164)

### New Features

- **When the Cisco Meraki API stops answering an organization, Yagra now says so — with one alert, not one per device.** A Meraki device is never pinged: what the Dashboard API reports is all Yagra knows about it. When a collect failed — a revoked key, a Dashboard outage, rate limiting, a poller that could not get out — nothing was published at all, so every device of the organization kept its last state (usually `ok`) and nothing alerted. Now, after **three availability collects in a row** go unanswered, one **critical** alert is raised about the organization (`Meraki organization “<name>”`), and it resolves when a collect is answered again — never merely because reports stopped arriving. The devices keep their last state, because they did not fail; each affected node's Overview says so: *“The Meraki API is not answering for <organization> (<reason>). The state shown here is the last one collected.”* The organization's row and page say which collect is failing and why. An uplink or traffic collect that fails while availability still answers raises nothing — readings are missing, not liveness — and is shown on the organization's page only. A folder-restricted account sees the alert when one of the organization's nodes is in a folder it can see.
  - A failed collect also no longer blocks **Sync now** for five minutes: the poller reports how every collect ended, which releases the organization's turn at once.
  - Older pollers send no such report. For them Yagra falls back to noticing that a collect it sent was never answered (`no_answer`), which takes the collect's five-minute lease per failure to notice, and cannot tell a failed collect from one that legitimately found no imported device.
  - Three things stop a collect before it is even sent, and no poller can report any of them: the organization's stored key cannot be opened, its imported devices cannot be read, or the networks it watches cannot be read. Its devices then go unasked-about exactly as they do when the Dashboard refuses, so Yagra counts these itself — at the collect's own interval, not at the scheduler's fifteen-second tick. An organization that deliberately watches no network is not one of them and raises nothing.
  - API: alerts gain the subject kind `meraki_org` (`node` is then `meraki_org:<organization id>`, `subject_name` the organization's name) on `GET /api/v1/alerts`, the alert stream and `GET /api/v1/alerts/history`. `GET /api/v1/nodes/{node_id}/status` gains `collection_fault` (`cause`, `meraki_org`, `meraki_org_name`, `reason`, `since_unix_ms`), present only while the node's state is not a current reading. `GET /api/v1/meraki/orgs` gains `collect_failures` (`tier`, `reason`, `since`, `failures`). The failure vocabulary gains `no_answer`. Over MCP: `get_active_alerts` and `get_alert_history` name the organization, `get_node_status` carries `collection_fault`, and `get_config kind=meraki_orgs` carries `collect_failures`. A client that switches on `subject_kind` should expect the third value.
- **A Meraki organization now imports its devices by itself.** Each sync (five minutes by default) turns a device into a node when it is in a watched network, Meraki has reported it online at least once, and it has never been a node here. A spare that is registered in the Dashboard but still in its box is left alone until it first comes online, so it does not arrive as a node-down alert; a device you delete stays deleted — put it back by hand from the organization's page. New devices are filed exactly as a manual import files them: into the folder whose IP range holds the address, otherwise under `Organization ▸ Network`. With automatic import on, a network the sync finds for the first time is watched from then on; a network you took out of scope stays out. **Organizations that already exist start with automatic import off** — every one of them was imported by hand, and switching it on would add each device somebody chose to leave out. **Organizations added from now on start with it on.** A device first becomes a node up to one sync interval after Meraki first reports it online, and gets its first readings at the following collection. (ADR-164)
  - Settings ▸ Integrations ▸ Cisco Meraki ▸ *organization* is a new page: the sync status, a **Device import** card (import automatically / file by IP range / the most devices to import), and every device Meraki lists with its state — Monitored, New, Never online, Deleted here, Not found in Meraki — and where it is filed or would be filed, with the reason (`Matches IP range 10.1.0.0/24`, `No matching IP range`, `2 folders match equally`, `No address`). Tick devices and press **Import** to bring them in by hand; that is how a deleted device, or one that has never been online, becomes a node. If automatic import is on while some networks are not watched, the card says how many and offers **Watch all**. The cap is never silent: when it leaves devices out, the card says how many. Importing devices by hand from the list also starts watching the networks they are in, as the old dialog did — except while automatic import is on, where watching a network would import everything else in it; there the page's notice offers that choice instead.
  - API: `PUT /api/v1/meraki/orgs/{id}/import-settings` (`import_devices`, `file_by_prefix`, optional `max_devices` 1–50000; ManageConfig, refused to an account restricted to folders). `GET /api/v1/meraki/orgs` gains `import_devices`, `file_by_prefix`, `max_devices` and `devices_over_cap`. `GET …/orgs/{id}/devices` gains `folder_id` and `filing { reason, prefix, folders }` — for a device that is not a node, where an import would put it right now. `POST …/orgs/{id}/sync` answers `imported` and `over_cap` as well. Automatic imports write no audit row; switching the setting does.
- **A Meraki organization's row now says what Meraki holds and whether the last sync worked.** Every five minutes Yagra reads each enabled organization's networks and devices from the Dashboard API and keeps the answer. Settings ▸ Integrations ▸ Cisco Meraki shows, per organization, when it last synced, `N of M devices monitored`, how many devices are **new** (Meraki has seen them online and they are not nodes here) and how many monitored devices Meraki **no longer lists**, with a **Sync now** button. A failed sync names its reason — the key was refused, Meraki kept rate-limiting, the API could not be reached, the device list did not arrive complete, and so on — and changes nothing else: a device is only ever marked as missing from a listing that was read to its end. A monitored device that disappears from Meraki keeps its node and its alerts. (ADR-164)
  - API: `POST /api/v1/meraki/orgs/{id}/sync` (ManageConfig; 409 while the organization or Meraki polling is paused or a collect is running, `meraki_sync_failed` when the sync ran and failed — 502 when the Dashboard API did not give a complete answer, 409 when the organization's stored key or base URL cannot be used, 500 when Yagra could not read or write its own database) and `GET /api/v1/meraki/orgs/{id}/devices` (View) — every device the last successful sync found, monitored or not, with its `state` (`monitored` / `new` / `never_online` / `deleted` / `missing`). Both refuse an account restricted to folders. `GET /api/v1/meraki/orgs` gains `last_sync_at`, `last_sync_ok`, `last_sync_error` and `devices { seen, monitored, new, missing }`. Over MCP: `get_config kind=meraki_devices org_id=…`, and the new fields on `kind=meraki_orgs`.
- **Imported Meraki devices are filed into the folder whose IP range holds their address.** "Import devices" used to put every device under `Organization ▸ Network`, so a deployment whose folders mirror its sites — made by hand, or by the NetBox sync — grew a second, parallel tree. A device whose address falls inside exactly one folder's IP range now goes into that folder, and the longest range wins. A device with no address, with an address no range covers, or with an address two folders claim equally goes under the organization's folder as before, and a network's folder is only created when a device actually lands in it. **File by IP range** is a setting of the organization, on by default — turn it off for one whose sites reuse one private range. After a manual import the page says how many devices went where, and says separately when overlapping ranges kept a device from being filed. (ADR-164)
  - API: `POST /api/v1/meraki/import` accepts `file_by_prefix` (absent means the organization's setting), and its 201 now also carries `filed { matched, ambiguous, unmatched, no_address }` and `ranges_configured`. `imported` is unchanged.
- **Add a Meraki organization under a key that is already stored.** "Add organization" now offers **Use a saved key** beside **Enter a new key**: pick a stored Meraki API key, find the organizations it can see, and add more of them without pasting the key again. The new organizations share that credential — nothing new is stored. Organizations already monitored here are listed as **Already added** and cannot be ticked, and each organization's row names the key it uses (for an account that can read credentials). (ADR-164)
  - API: `POST /api/v1/meraki/orgs/discover` and `POST /api/v1/meraki/orgs` accept `credential_id` in place of `api_key` — exactly one of the two. Both is `400 invalid_request`, neither is `400 invalid_api_key`, and a `credential_id` that is not a stored Meraki API key is `400 invalid_credential`, refused before anything is sent to Meraki. `discover`'s entries gain `already_added`. `POST /api/v1/meraki/orgs` skips organizations that are already here and answers `already_added` beside `created`. `GET /api/v1/meraki/orgs` gains `credential_id` — the credential's id, never the key. Over MCP: the same field on `get_config kind=meraki_orgs`.
- **Folders made by an integration are marked in the tree.** A Meraki organization's folder and its network folders carry a `Meraki` badge, and a Region or Site folder the NetBox sync created carries `NetBox`. They do not behave like a folder somebody made: a Meraki organization's folders are deleted with the organization, and a NetBox folder is renamed back by the next sync. A folder you create inside an organization's folder is yours and is not marked, and forgetting a NetBox server leaves its folders behind, unmarked. (ADR-164)
  - API: `GET /api/v1/node-groups` gains `origin` (`meraki`, `netbox` or `null`). Over MCP: `list_node_groups` carries it when it is set.

### Improvements

- **Meraki's "Inventory interval" is now the device sync interval, and it is much shorter.** It was a setting for a tier that never ran (default six hours, minimum 15 minutes). It now decides how often an organization's device list is read, so the default is **five minutes** and the minimum **one**. An organization still holding the old default of 21,600 seconds is moved to 300; any other value was typed by someone and is kept. ⚠️ **Dashboard API usage goes up slightly, and for some organizations from zero:** each enabled organization now makes three more paged requests per sync — 36 an hour at the default, against Meraki's limit of 10 a *second* per organization — and this includes organizations with no imported devices, which until now were never called at all. A sync and a metric collect of the same organization never run at the same time, and the Meraki polling kill switch stops both.
...
- **A Meraki device that is deleted and imported again comes back as the same node.** A newly imported device's node id is derived from its serial, so its charts and alert history are its own again instead of starting empty. Devices imported before this keep the id they have. (ADR-164)
- **An imported Meraki device's node now follows the Dashboard.** Every sync compares each imported device's node with what Meraki reports. **The address** is corrected whenever Meraki reports a usable one that differs — until now a node kept the address it was imported with forever, and no screen could change it, so a device on DHCP stopped matching its own syslog, traps and flows, and a device imported before it had an address stayed at `0.0.0.0`. **The name** follows a rename made in the Dashboard, but only while the node still carries the name Meraki gave it: a node an operator renamed keeps that name. The node is never moved to another folder, whatever network the device moves to. ⚠️ **The first sync after upgrading re-addresses every imported Meraki node whose address had drifted**, all at once; polling, charts and alert history are unaffected, because they are keyed by the node, not by its address. A name that drifted before this release is *not* corrected — it cannot be told apart from a name someone chose. Only the core needs upgrading. (ADR-164)
  - API: the report of `POST /api/v1/meraki/orgs/{id}/sync` gains `followed`, the number of nodes that took something from the Dashboard in that sync.
- **A monitored Meraki device that nothing is being collected for is now pointed out.** Collection asks the Dashboard about watched networks only, so a device that is moved into an unwatched network — or whose network someone stops watching — silently keeps the last state it was seen in and raises nothing. The organization's row now says how many such devices there are, and its page says why and offers **Watch these networks**, which watches only the networks those devices are in. No alert is raised for it. (ADR-164)
  - API: `devices` in `GET /api/v1/meraki/orgs` gains `monitored_unwatched`.

### Bug Fixes

- **A Meraki organization whose collect could never start raised nothing, if its tiers happened to be stored with `availability` second.** The scheduler picks the most overdue tier, and until one has been dispatched they are all equally overdue — so it kept the first tier in the order the organization's `enabled_tiers` were stored in. While something stopped every collect before a job was sent (a key that cannot be opened, a database this core cannot read), the failure was therefore counted against that first tier and no other, and availability is the only tier whose failures raise the alert that says the organization is not being collected. The scheduler now takes the tiers in a fixed order — availability, uplink, traffic — whatever order they were saved in. The WebUI has always saved `availability` first, so only a client calling `PUT /api/v1/meraki/orgs/{id}/cadence` directly could reach this. (ADR-164)
- **One failed database read could send ICMP and SNMP polls to every Meraki node for a round.** The scheduler reads which nodes the Meraki collector owns once per round and skips them. When that read failed it fell back to an empty set, on the strength of a comment saying a later stage would turn Meraki nodes away anyway — the scheduler's path deliberately has no such stage. A Meraki node's address is `0.0.0.0` whenever the Dashboard reports none, so for that round each one was probed at an address nothing answers on, and the failures counted toward its node-down alert. A failed read now keeps the last set, and the set is read after the node list rather than before it, so a device imported between the two reads is never polled as an ordinary node. (ADR-164)
...
- **Importing Meraki devices now says how many became nodes.** The endpoint has always answered with a count and the import dialog discarded it. Devices that were already monitored are skipped, so the count can be lower than the number ticked — that is now visible instead of silent.
...
- **Two imports of the same Meraki device at once no longer fail.** Which serials were already monitored was read before the import began, so the slower of two imports hit the serial's uniqueness constraint and answered 500 with nothing imported. The check now happens inside the import, under a lock, and the device is skipped. A serial already monitored under another organization is skipped the same way. (ADR-164)
- **Deleting a Meraki organization's folder no longer leaves later imports unfiled.** Once that folder was gone, every newly imported device landed at the top of the tree in no folder at all, with nothing saying why. The next import now puts the organization's folder back and files under it. (ADR-164)
- **A Meraki device the Dashboard reports offline is now shown as down, and alerts.** Every Meraki result used to be reported as reachable, whatever the Dashboard said, so an offline or dormant device stayed `ok` on every screen and raised nothing. Its state now follows the availability check — the same reading as `meraki_device_up` — and the uplink and traffic collections no longer speak for whether the device is up, so a healthy uplink reading cannot cancel an outage. Thresholds on uplink and traffic metrics are judged as before. **After this upgrade, imported Meraki devices that are offline right now will start alerting**, after three consecutive availability checks (about 15 minutes at the default 5-minute interval). The change is in the poller: a remote poller still on an older version keeps the old behaviour until it is upgraded. (ADR-164)
- **Deleting a credential that a Meraki organization or a NetBox server uses answered 500.** Those two references cannot be cleared the way a node's binding is — an integration with no key is not something its row can express — so the database refused the delete, and the refusal reached the client as an internal error that named nothing. It is now `409 credential_in_use`, and the Credentials page no longer offers a delete that will be refused: it says what still uses the credential and where to remove it. The **Used by** column counts Meraki organizations and NetBox servers as well as nodes, so a Meraki key that three organizations are polled with no longer reads `Unused`.
  - API: `GET /api/v1/credentials` gains `used_by_meraki_orgs` and `used_by_netbox_servers`. `used_by` still counts nodes only.
- **Replacing a Meraki API key from Settings ▸ Credentials broke the organization it belonged to.** The edit dialog drew its four-type select over a key it had no option for, so it showed *SNMP v2c* while holding `meraki_api`. Left alone, the key was stored as typed where a JSON document belongs; touched, the key became an SNMP community. Either way every collect and sync of the organization failed with `credential` from then on, and the dialog is the only place a rotated key can be entered. A key an integration owns now keeps its type — the dialog says which it is instead of offering others — and is sent in the shape the server stores. A NetBox token is handled the same way.
  - API: `POST /api/v1/credentials` and `PUT /api/v1/credentials/{id}` now answer `400 invalid_credential` for a `meraki_api` secret that is not `{"api_key": "…"}` and a `netbox_token` secret that is not `{"token": "…"}`, as they already did for a malformed SNMPv3 or HTTP-auth document. Before, such a secret was stored and failed later where only a log said why.
- **A Dashboard API answer that is not a list is no longer read as an empty organization.** Every listing Yagra reads from Meraki is a JSON array. A `200` carrying anything else was taken as one unreadable item, so the listing read as complete and empty: the inventory sync marked every device of the organization as no longer listed until the next good sync, and a collect counted as answered — which is what closes an organization's collection alert. It is now a failed read (`malformed`), for the sync and for every collect.
- **Saving a Meraki organization's import settings no longer flicks the form back to the old values.** The form went back to following the server at the moment of the save, while the page still held the previous settings, so what had just been saved was replaced by what it replaced until the reload finished. It now waits for the reload, and stays locked while it runs.
- **Ticking a device on a Meraki organization's page no longer re-filters the whole list.** Each tick rebuilt the table's columns, and the filter row recomputed every device and every filter count from them — three passes over an organization of thousands per click.
- **The Meraki and NetBox sync times follow the interface language.** They were formatted in the browser's locale, so a Japanese interface on an English browser read `9/20/2026, 12:00:00 AM` in the middle of a Japanese sentence.
- **An enabled Meraki organization is no longer drawn in the "OK" green.** The status colours belong to node and alert states; a green *enabled* on the organization's row read as a device that is up, and the organization's own page already said the same thing in plain text.
- **The cadence dialog's ranges can no longer drift from what the server accepts.** The four ranges were typed into the page by hand; they now come from one list that a test holds to the server's bounds, and each interval's input carries them as its minimum and maximum.
- Three published descriptions were untrue and are corrected: `filed` in the answer of `POST /api/v1/meraki/import` is all zeros whenever filing by IP range was off for that import, including when the request left it to the organization's setting; the MCP `get_config` tool's `meraki_devices` names the `missing` state it returns; and the device states no longer cite the wrong design decision.
- **Adding a Meraki organization that was already there stored another copy of the key.** The key was sealed as a new credential before the organizations were created, so a batch in which every organization already existed left a credential on the Credentials page that nothing used. Organizations that are already here are now taken out first, and a key that ends up backing nothing is not kept.

### Security

- **A folder-restricted Operator could re-seal or delete credentials that other folders' devices are polled with.** Credentials are not filed in any folder — one community string or one API key is what every site is polled with — and the credential endpoints required `ManageCredentials`, which Operators hold, without ever looking at the account's folder restriction. An Operator limited to one site could therefore replace or delete the credential another site runs on, and every device using it would stop answering; for a Meraki API key the organization's nodes go quiet rather than alert. No secret could be read this way — the API never returns one — and every such write was audited. The three writes now refuse a restricted account. Found by reading the code, not by a report of misuse. (ADR-158)
...
- **The inventory tree can show just the nodes that need attention.** "Needs attention", beside "With nodes" on All nodes, narrows the tree to the nodes that are warning, critical or unreachable — the same three the "N need attention" count under the page title has always named, and that count is now a second way to press it. It is a shortcut over the State filter rather than a filter of its own, which is what makes it explainable: pressing it ticks those three states in the State control, so you can see what it did and take one back out by hand; "Clear all filters" undoes it; and it travels in the URL, so a narrowed tree can be shared or bookmarked. Pressing it again clears the state filter rather than restoring whatever was selected before. A node down at the moment counts as "unreachable", not "critical" — all three states are included for exactly that reason. A node whose maintenance window is open is **not** included: while the window lasts its state is "maintenance", which is what the window is for. Pressing the header count while the inventory pane is collapsed to a rail opens the pane, so the narrowed tree is on screen.
- **An access point’s radios now appear as ports on its node.** A node on the built-in "Huawei wireless controller" profile picks up a third built-in template, "Huawei WLAN radios (AC)", on the next core start. Every access point it has imported gains one **Interfaces** row per radio — slot 1 is 2.4 GHz, 2 is 5 GHz, 3 is 6 GHz, and a second radio in a band is 11, 12, 13 — carrying `wlan_radio_client_count`, `wlan_radio_channel`, `wlan_radio_channel_util_pct`, `wlan_radio_interference_pct`, `wlan_radio_noise_dbm`, `wlan_radio_client_signal_dbm` and `wlan_radio_tx_power_dbm`. Air traffic and the up/down state are published under the ordinary interface names (`if_hc_in_octets`, `if_hc_out_octets`, `if_oper_status`), so a radio draws on every screen that already draws a port — and **a threshold rule can name one radio of one access point**, at the interface tier, exactly as it can name a switch port. The Interfaces tab now appears on access-point nodes, which have no SNMP credential of their own; Neighbors and Flow stay hidden, because a controller has no equivalent to fill them. Readings the controller marks invalid are absent rather than zero: a noise floor of 0 dBm would read as a radio being drowned, and a 0 dBm client signal as the strongest possible. The reported channel width is deliberately **not** stored as a port speed — 20 MHz is not a line rate — so a radio shows throughput without a utilization percentage. ⚠️ **Radios roughly double the size of a controller’s report.** Measured: 1,024 access points with two radios each is 797 KB, which fits; 2,048 is about 1.4 MB, which exceeds what one message may carry at any setting. A controller's report is therefore cut at 1,024 access points, and its `aps_truncated_at` then says how many it reported. Upgrade the poller as well as the core; an older poller does not walk the radio table.
- **Access points report their CPU temperature and how they are being powered.** `wlan_ap_temp_c` reads the controller’s operating-temperature column, which the great majority of access points do not implement — measured on a Huawei AC6508, 36 of its 38 APs answered the vendor’s "no sensor" placeholder and only 2 a reading. Two readings are added beside it, taken from a **second** walk of the AP table: `wlan_ap_cpu_temp_c`, the CPU die temperature, which 30 of those same 38 answered (55–69 °C); and `wlan_ap_power_state`, how the access point is being powered (1 normal, 2 insufficient, 3 limited), recorded only while its controller is serving it, so a down access point has no value there. `wlan_ap_temp_c` keeps its meaning exactly — they are different sensors and one is never substituted for the other. The second walk is what makes this safe to ship: a column a controller does not implement now costs that reading alone, where adding it to the existing walk would have emptied the whole AP list on any model that skips it. Upgrade the poller as well as the core; an older poller sends neither reading and both read no data.
- **A wireless controller now reports its SSIDs, and how many clients are on each one.** A node on the built-in "Huawei wireless controller" profile picks up a second built-in template, "Huawei WLAN SSIDs (AC)", on the next core start. It walks the controller’s SSID statistics table and records one row per SSID, named by the SSID itself: `wlan_ssid_clients` and its per-band parts `…_2g4`, `…_5g`, `…_6g`, `wlan_ssid_ap_count` (how many access points broadcast it), and the raw byte counters `wlan_ssid_in_octets` / `wlan_ssid_out_octets`. The controller also gains `wlan_controller_ssid_count` and `wlan_ssid_walk_complete`. Because the rows are ordinary metrics, each SSID has a history you can chart and a row you can write a threshold rule against — `wlan_ssid_clients` with a row pattern naming one SSID. The SSID name is **not** a series label: it is stored beside the row key and joined when the value is read, so a fleet of controllers costs one series per SSID rather than one per name ever seen. An SSID nobody is connected to still appears, with zero. Detaching the template stops the walk. ⚠️ **An HA standby reports the active controller’s client counts**, exactly as it does for the AP and client totals, so the two members of a pair show the same figures — do not add them together. ⚠️ The send and receive **rates** the controller also offers are not collected: the MIB gives them no unit, and the byte counters answer the same question through `rate()`. Upgrade the poller as well as the core; an older poller does not walk the table and the SSID metrics read no data.
- **The inventory tree can hide folders with nothing in them.** "With nodes", beside "Pinned only" on All nodes, drops every folder that has no node in it or in any folder below it; the folders above a node stay, so nothing loses its place. It judges by the whole fleet's membership rather than by the folders you have opened, combines with the search box and the state / kind / pool filters, and — like "Pinned only" — is remembered on your account. A folder you create on that screen stays visible until you reload, even though it is empty.
- **Access points behind a wireless controller can become nodes of their own, from the controller's own page.** A controller that has reported an AP inventory gains an **APs** tab: every access point it reports, with its state, address, model, clients, which controllers see it and whether it is monitored — searchable and filterable — plus a **Monitor** button on each one that is not yet a node, and a panel that turns automatic monitoring on, sets the limit and picks the folder. Both need the Manage configuration privilege and are not drawn without it; the tab appears only once the controller's first AP inventory has arrived. The same two writes are available directly: `PUT /api/v1/nodes/{node_id}/wireless-controller` with `{"import_aps": true}` switches it on for one controller (off by default; `max_aps`, 1–2048, default 1024, caps how many; `ap_group_id` picks the folder, otherwise they are filed in the controller's own folder — no folder is created for them, and an AP node already imported is never re-filed). Within a minute every AP that controller reports and that has ever been in service becomes a node of the new kind `wireless_ap` (badge "AP"), with the AP's name, address, maker and model; an AP that has never been in service stays in the AP list and can be imported one at a time — the tab's **Monitor** button, or `POST /api/v1/wireless/aps/{ap_id}/import`. APs the cap leaves out are counted in the controller's `aps_over_cap`. An AP node is never polled itself: when the controller serving it reports it in service it records `wlan_ap_up` = 1 with `wlan_ap_client_count`, `wlan_ap_cpu_pct`, `wlan_ap_mem_pct` and `wlan_ap_temp_c`, and when that controller reports it down the node goes unreachable and raises the usual node-down alert — one alert per AP. Behind an HA pair the standby's view (its zeros included) is never recorded and cannot take an AP down, and an AP keeps its node, id and history when the pair switches over. An AP that no controller reports any more is not marked down; its values simply stop arriving. The node's name follows the controller's until someone renames the node, and its address follows the one the serving controller reports. A deleted AP node is not imported again automatically. `GET /api/v1/nodes/{node_id}` and the MCP tool `get_node_status` gain `wireless` (the controller's inventory and import settings, or the AP's entry), `kind` gains `wireless_ap`, and an AP's Overview replaces "Polled by" with "Wireless controller", naming the controller serving it and linking to it (elsewhere an AP's "Polled by" reads "Its wireless controller"). ⚠️ A configuration bundle exported from a deployment with AP nodes carries them as ordinary nodes, which the importing deployment would ping. A folder-scoped account imports an AP only through a controller in its folders and only into a folder it can see; otherwise the request is refused with 409 `ap_destination_out_of_scope`. ⚠️ **Known gaps in this release:** a `max_aps` above 1024 has no effect yet — a controller's inventory is still cut at 1024; when a controller stops answering, its APs' nodes keep the last state they had (usually `ok`) instead of turning `unknown`; and while an AP is down, a threshold alert on one of its readings (CPU, memory, temperature) is closed after about six hours as though its data had merely stopped.
- **Wireless controllers list the access points they manage.** A node on the "Huawei wireless controller" profile now walks the controller's AP table (HUAWEI-WLAN-AP-MIB) on every poll, and `GET /api/v1/wireless/aps` returns each AP's name, MAC address, model, software version, serial, IP address, state (`associated`, `backup` or `not_associated`) and online clients — across every controller, or for one with `controller_node_id`. The MCP tool `list_wireless_aps` answers the same question. An AP behind an HA controller pair is listed once, with the state the controller serving it reports and every controller's view in `reported_by`; the standby's view never replaces the active's. An AP that stops being reported is not removed — its `last_seen` stops advancing. The controller gains one metric, `wlan_ap_walk_complete` (1 when the AP table was read to its end), and one default rule on the profile: a warning when it stays below 0.5 for three polls, which means the AP list has stopped refreshing. Upgrade the core and the poller; an older poller skips the AP walk and `wlan_ap_walk_complete` reads no data. A folder-scoped account sees an AP when a controller in its folders reports it, and sees only those controllers: `controller_node_id` and `reported_by` name no controller outside its folders, `node_id` is `null` for an AP node filed outside them, and `controller_node_id=` naming a controller outside them matches nothing. ⚠️ **Measured on controllers with up to 38 APs.** The AP table is read column by column within 16 seconds (eight SNMP timeouts); by calculation, a controller with more than roughly 800–1,200 APs — 300–500 over a slow link to a remote site — may not finish in time, and its AP list then never refreshes. `wlan_ap_walk_complete` stays below 0.5 and its default rule warns.
...
- **Dragging in the inventory tree now shows you what will land, and where, as a row of its own.** The 2px insertion line has been replaced: while you drag, a dashed row carrying the name of what you are holding appears at the exact position — and, crucially, at the exact indentation — the drop will write to. The line could not say which folder it meant. A line under a folder's last device and a line over the next folder's row are one pixel row apart and land in different parents; now the first is drawn indented among that folder's devices and the second is drawn beside the folder, and the destination folder's own row is outlined dashed as well. Dropping *into* a folder (the middle of its row) is unchanged: the row is outlined, because that placement appends to the end of the folder's contents, which is often off screen — a marker there would appear somewhere you are not looking. The rows you are dragging stay where they are, dimmed, until you let go, and the slot is itself a drop target, so releasing on the marker does what the marker says. A batch of nodes shows the row you grabbed and a count of the others travelling with it. Nothing about where a drop actually lands has changed, and neither has the API.
- **Folders and nodes in the inventory tree are now one ordered list, so a folder can be dragged anywhere among a folder's contents — including between two nodes.** Dropping a folder onto the rows a folder's nodes occupy used to be refused outright, which left only the thin band on a folder's own row as a target; the more devices a folder held, the less of it you could aim at. A folder now lands where it is dropped. Three consequences worth knowing. **The right-click Sort ascending / descending now orders a folder's subfolders and its nodes as a single A→Z list**, so a subfolder named `m` falls between the nodes `k` and `p` — it used to renumber the two separately, which left every folder above every node; as before, it replaces an order arranged by hand and there is no undo. **A folder synced from NetBox now keeps the position you give it**, because a sync sets a folder's position only when it first creates the folder; re-ordering sites in NetBox no longer reaches Yagra. **Dropping a folder among the Ungrouped nodes at the top level is still refused** (they are drawn apart from the top-level folders, so the folder would appear somewhere other than where it was dropped). The upgrade renumbers each folder's contents once and preserves exactly the order on screen today, so nothing moves when you upgrade. `PUT /api/v1/node-groups/{id}/placement`'s `before`/`after` may now name a node as well as a folder; an id that names neither still appends.

### Bug Fixes

- **Discovery tries every credential you select, not just the first three.** A sweep probed each target with at most three candidate credentials: after three that did not answer, the per-device limiter put the target into a 15-minute cooldown and the rest were dropped without a packet being sent or a line being logged. Because the Discovery screen ticks every stored SNMP credential by default, a deployment with four or more credentials could fail to find a device whose community it holds. All selected credentials are now tried in order. The order is the one the chips are shown in — it used to be the order they were clicked, so the second credential on screen was not necessarily the second one attempted. ⚠️ A sweep now costs one attempt per selected credential on each target that reaches the SNMP stage, so selecting many credentials makes a sweep slower; with "Also try SNMP on addresses that do not answer ping" ticked, expect roughly 1.6× the previous time for a /24 with five credentials. The number chosen is logged when the sweep starts. The removed limit protected nothing: SNMP has no account to lock out, and the limiter was rebuilt for each target, so its cooldown never outlived one target's attempts.
- **A slow SNMPv3 device is no longer mistaken for a wrong credential.** When the first identity read timed out, the session kept reading: the late reply arrived as the answer to the *next* request, failed its request-id check, and was recorded as "the device answered". Every remaining value came back empty, so a correct v3 credential scored as a failure — during discovery that also consumed one of the attempts above. A read now stops at the first timeout.
- **An upgrade no longer closes and re-opens the incident of every device that is still down.** Settings ▸ Upgrade opens a fleet-wide maintenance window while it restarts, and every check inside a window used to be observed as `maintenance` — which, for a device that was already down, resolved its node-down alert and then raised it again about five minutes later, with its `snmp_up` and ICMP-loss alerts paging in between. With PagerDuty or Jira Service Management connected that was one close and one re-open per down device per upgrade. The window now **pauses** judgement instead: nothing is judged while it is open, so open alerts stay open, no new alert fires, and when it ends each check carries on from the state it had. A device that recovered during the upgrade still resolves afterwards, and one that failed during it still fires, each after the usual dwell. Nodes read their last known state during the window rather than `maintenance`, and stop reading `maintenance` for several minutes after it. ⚠️ **Upgrading *to* this version can still show the old behaviour once**, because the version that opens the window is the one being replaced. The same applies to the window opened by Settings ▸ Pollers ▸ "Accept remote pollers".
- **Opening a maintenance window on a device that is down no longer notifies about the alerts that window is meant to silence.** Its `snmp_up` and ICMP-loss alerts had been rolled into the node's own outage; when the window took the node out of the down set, they were un-rolled and delivered as fresh notifications, then resolved a poll or two later. A window on an upstream device did the same for everything depending on it. Leaving a window still re-evaluates the roll-up, so a device that answers ICMP again while its SNMP agent stays dead pages for `snmp_up` as it should.
- **A database read that fails while core is starting no longer closes the alerts of devices that are down.** If the first load of the alert configuration failed, the engine held no rules, which reads the same as "the node-down rule was deleted" — so every alert restored from history was closed on the first poll, and the device stayed down with nothing left to re-open it. An engine that has never loaded a configuration now closes nothing; deleting a rule still closes what it stranded.
- **All nodes no longer re-fetches its maintenance and mute lists in a loop when the nearest expiry is more than 24 days away.** The page waits for the next maintenance window, mute or exemption to run out and then reads the three lists again. A browser timer cannot hold a delay above 2^31−1 ms (24.8 days): it fires at once instead. With one quarter-long mute and nothing expiring sooner, the timer fired, the lists were read, the fresh answer re-armed the same timer, and the page issued three requests per round trip for as long as it stayed open — from every open tab. The delay is now capped, so the page re-reads once every 24.8 days at most and then waits again.
- **A dashboard that fails to load can no longer be overwritten with the default board.** When the read of My Dashboard or the Shared dashboard failed once — a timeout, a 5xx, a restart in progress — the page showed the five-widget default as if it were your own board, with Customize live beside it. The first edit then saved that default over every board you had built. A failed load now shows "could not be loaded" with a Retry button and no Customize, and nothing is written until a load has succeeded. Separately, leaving the dashboard and returning within a second of an edit no longer loses it: the pending save is sent before the board is read again, and an edit made while a read is in flight is not replaced by that read's answer.
- **The public dashboard's alert widgets show the alerts that are already open, not only the ones that fire while you watch.** Active alerts, Severity mix and Flapping watchlist are fed by a live stream, and the stream carries only what changes after it connects. The alerts open at that moment come from a separate read — and the widgets declared only the stream, so on a public board that read was refused for an anonymous visitor and the refusal was discarded. A visitor arriving in the middle of an outage saw "No active alerts". The three widgets now declare both routes, so placing one on the public board opens `GET /api/v1/alerts` to anonymous visitors as well (the same alert records the stream already carried; nothing new is disclosed). A public board with no alert widget no longer opens the stream at all — it was being refused and retried every three seconds by every visitor.
- **A read that fails is no longer shown as a healthy answer in five places.** The dashboard's *Nodes down*, *Health ring* and *Status summary* tiles drew `0`, "No nodes yet" and an empty tally when `/fleet/summary` failed; they now say the summary could not be loaded. A finished Troubleshoot analysis whose findings could not be fetched drew a complete report reading "0 critical · 0 warning" with nothing found, and never asked again; it now shows the error with a Retry button, and switching from one run to another no longer shows the first run's findings under the second until they arrive. A node's Collection tab painted a healthy SNMP device red with "Collection failing" whenever the node's metric list could not be read, and the Overview dropped its whole Device health section without a word; the tab now says the status is unavailable, the Overview says why the cards are missing, and the card no longer flashes "Collection failing" for a moment while it is still loading.
- **A node's page no longer shows the previous node's ports, and Poll now no longer blanks the pane.** On the full-page node view (`/nodes/<id>`), jumping to another node from the search box or with Back/Forward kept the first node's interface list on screen under the second node's name until the new list arrived — and indefinitely if that read failed. The page now starts each node from an empty pane, as the All nodes split view always did. Separately, pressing **Poll now** re-read the node 4 and 30 seconds later by rebuilding the whole pane: it fell back to "Loading…" twice, an open interface dock closed twice, and the wireless controller's AP tab lost its "imported" confirmation before it could be read. Those re-reads now happen in place, and they are cancelled when you leave the node instead of firing into the next one.
- **A slow event search can no longer replace the results of the search that came after it.** Clearing a message filter that was still running — a regular expression over the log store can take tens of seconds — showed the full list at once, and then the abandoned search finished and overwrote it: a filter row showing no filter, over a list that was still filtered, with nothing left to refresh it. The same applied to a *Load more* page asked for under a filter that had since changed. Both are now discarded when they arrive. Two smaller cases of the same kind: Settings ▸ Pollers no longer shows one poller's node list under its own name while you wait for the poller you just clicked, and signing out while your layout preferences are still loading no longer applies them to the next account that signs in on that browser.
- **Eight places where a failure was silent, or said the wrong thing.** After a core restart drops its sessions, a download — a poller's token kit, the support bundle, a report export, the relocation archive — answered 401 into its dialog, left the dead session in place and failed identically on every retry; it now returns you to sign-in like every other request. Dashboard ▸ Reports ▸ Saved reports and Troubleshoot ▸ Runs stayed on "Loading…" for good if their first read failed; both now show the error with a Retry button. The Reports templates and schedules tabs answered a failed first read with "No report templates yet" and a disabled *New schedule* telling you to create a template you already had; they now show the error. A report that was still generating stopped updating for good after one failed poll; it keeps polling. **Run now** on a report template failed without a word; it now says why. The NetBox enable switch snapped back without a word when its write was refused; the row now shows the reason. The dashboard's Audit widget told every failed read — including an admin's — "you are not permitted"; it says that only for a 403. And the URL monitor's Availability tile, the DNS monitor's Resolution tile and the Meraki device's Availability tile drew the `—` of "no reading yet" in the failure colour; it is now the unknown colour.
- **A dialog no longer closes when a text selection ends outside it.** Dragging to select text in a dialog's field and releasing the button a few pixels past the dialog's edge closed the dialog and discarded everything typed into it — most painfully a pasted CA certificate or an API token in the NetBox and Forwarding dialogs. A browser reports that gesture as a click on the backdrop. A dialog now closes on the backdrop only when the press both starts and ends there.
- **Confirming a Japanese conversion with Enter no longer commits the field.** Text is typed in Japanese through an input method, and Enter confirms the conversion it offers — but the page received that key as well. The node, folder, scope and metric pickers selected whatever row was highlighted for the half-typed text and closed; the label field stored the unconverted text as a label; the top-bar search opened the first result; a board or widget name was committed mid-word. Ten fields now ignore Enter and the arrow keys while the input method is composing. This affects macOS (Chrome and Safari) most; Chrome on Windows reports that key differently and was largely unaffected.
- **Four forms that lost or rewrote what you gave them.** Signing in no longer throws away the address you were sent to: a deep link opened without a session showed the sign-in form and then always landed on the shared dashboard, so a link pasted into chat during an incident was lost by everyone who had to sign in first. Settings ▸ Pollers ▸ *Issue token* now updates itself after issuing: the dialog stays open by design, but it went on describing the poller as it was before — still "uses the shared secret", still no **Revoke**, and pressing the button again re-issued without the warning, invalidating the token you had just handed to the site. The event-alert-rule dialog checks *Clears after*, *Fire after* and *within* before saving and says which box is out of range and what it may hold; a cleared box used to be sent as `0`, and the server's refusal arrived as a raw English sentence. And opening a report schedule whose frequency this version does not know no longer rewrites it to daily on Save — the frequency stays unset until you choose one.
- **Troubleshoot no longer offers "Run" to an account that cannot run an analysis.** Starting or cancelling an analysis needs the *Acknowledge alerts* privilege; reading past runs needs only *View*. A Viewer nevertheless saw a Run button on all fifteen tools, *Run analysis* and *Re-run* on every report, *Cancel* and *Retry* on every run, and *Edit* / *Delete* on every scheduled analysis — and each of them failed with "could not start" or a refused save, with no reason given. Those controls are now drawn only for an account that may use them, and the catalogue and the report page say which privilege is missing instead.
- **Two display slips.** A rate or size just under a unit boundary printed as 1000 (or 1024) of the smaller unit — `1000 Mbps` for a gigabit link running at line rate, `1000k` on a chart's axis, `1024 MB` for a memory total one byte short of a gibibyte. The formatters picked the unit first and rounded second; they now round first, so those read `1.0 Gbps`, `1.0M` and `1.0 GB`. And the port-rule dialog's "Stored as …" line, which exists to show what the rules screen will list, always read `—`: it looked the number up under the old field names, which this dialog stopped writing when two-sided rules arrived. The rule itself was always saved correctly.
- **All nodes left open no longer goes stale.** The folder health bars, the node total in the header and the "N need attention" count were read when the page opened and after a change you made, and never again — while the live stream went on recolouring the dots beneath them, so a wall display showed red dots under a green bar. They are now re-read every minute. When the live stream reports that it fell behind — or drops and reconnects, which misses whatever happened in between — the page now re-reads the states it shows instead of ignoring the notice, and keeps the tree on screen while it does; until now a node whose recovery was in the missed part stayed red for as long as the tab was open, because the live state overrules every later read. Also on this screen: a search that fails now says so with a Retry button instead of drawing an empty tree, which under **Needs attention** read as "nothing needs attention"; the header no longer shows the handful of nodes it happened to have loaded as the fleet total when the summary cannot be read; and deleting a folder before its node count has arrived says its member nodes are still being counted instead of "0 member nodes".
- **A node moved, deleted or re-pooled while its folder was still loading no longer reappears in its old folder.** All nodes loads each folder's members as the folder scrolls into view. If you changed something while one of those reads was still on its way, its answer — describing the inventory as it was before your change — arrived afterwards and was kept, and that folder was never asked again: the moved node was drawn under both folders until the page was reloaded. An answer asked for before a change is now discarded, and the folder is read again.
- **Three dialogs on All nodes.** The node pane's **Move…** button did nothing when the selected node was not among the rows the tree had loaded — which is the case for a link that opens a node directly, and for a selection the active filter no longer matches. It now moves the node the pane is showing. **Set dependency** no longer clears an upstream it could not show you: when the dependency map failed to load, the dialog displayed "No upstream" for a node that had one, and pressing Save removed it. The current upstream now stays selected, and the dialog says the map could not be loaded. And **Add node** no longer lets you submit a pool name it has already marked invalid.
- **Three smaller fixes in the inventory tree.** A node released on a folder's "Loading nodes…" or "could not load" row now goes into that folder; those rows are drawn inside the folder and read as part of it, but refused the drop, so the node went nowhere while the folder above stayed outlined. Opening a row's menu from the keyboard (the Menu key or Shift+F10) when that row is scrolled out of view now opens it beside the row; it used to open at the corner of the tree pane. The row buttons (add, edit, delete, open) are now always visible on a touch screen, where there is no hover to reveal them and, on an iPhone or iPad, no long-press menu to fall back on.
- **Five screens drawn with values that did not exist.** The NetBox integration screen used spacing and text sizes defined nowhere, so its gaps and padding collapsed to nothing; the small print on the authentication settings, TLS settings and shared dashboard screens took the size of the text around it; and panes on a node's Flow tab and on the Geo map lost their rounded corners the same way. They now use the spacing, sizes and radius every other screen uses. And the table on a node's **Neighbors** tab stays inside its box and scrolls there, instead of growing to the height of every row and making the whole tab scroll.
...
- **Threshold rules on an optical light level, and on the chassis temperature the optical probe reads, now raise alerts.** The optical probe's result is marked as saying nothing about whether a device is up, and core skipped the alert engine entirely for such a result — so a rule on `if_rx_power_dbm`, `if_tx_power_dbm` or a Cisco chassis temperature was stored, listed and never checked, and never fired. Its readings are now checked like any other sample; its outcome still never affects a node's up/down state. ⚠️ **Existing rules on these metrics will start firing** for readings already outside their bounds. The neighbour, address, ARP and routing walks run far less often than a node's poll and are still not checked. Upgrade the poller and the core; either one alone changes nothing.
- **A Cisco-style chassis temperature is no longer reported from a transceiver's own sensor.** The optical probe tells a chassis sensor from an SFP's by whether ENTITY-MIB attaches the sensor to a port. When that table could not be read in full — a timeout, a missing column, or a table cut at its row limit — every sensor looked unattached, and each module's own temperature was published as a chassis temperature beside the real one. A poll whose ENTITY-MIB read is incomplete now publishes no chassis temperature at all (a gap in the chart); light levels still map through whatever part of the table was read. Series already stored remain until they age out. The poller counts the reads as `yagra_optical_entity_index_walks_total{result="complete"|"incomplete"}`.
- **An alert open for longer than the alert-history retention period is still restored after a restart.** Pruning deleted every history record older than the retention setting, including the record of an alert that was still open. The next core restart then did not know that alert was open: it fired again as a new alert, and if the problem had cleared while core was down, the incident it had opened in PagerDuty or Jira Service Management was never resolved. Pruning now keeps each check's most recent record while that record is an open alert; everything else is pruned as before. A record already deleted cannot be recovered, so an alert that has been open longer than the setting will fire once more at the next restart. A new index on `alert_history (check_id, at_unix_ms)` is created on upgrade.
- **Renaming a pool no longer leaves it briefly unpolled.** The rename moved every stored reference to the new name, but core's in-memory record of which poller serves which pool kept the old one until each poller's next heartbeat. An assignment pass in that window found no poller for the renamed pool, sent its poller an empty work list and routed its nodes to a subject nothing listens on. Core now updates its record in the same request, as it already did when a single poller is moved to another pool.
- **A trap whose OID is longer than an OID may be is refused rather than stored.** SNMP allows at most 128 sub-identifiers, each at most 32 bits. A trap identity (`snmpTrapOID.0`, or a v1 trap's enterprise) beyond that is now dropped and counted as `yagra_events_dropped_total{reason="parse_error"}`, and a varbind whose name is beyond it is left out while the rest of the trap is kept. Nothing is shortened, because a shortened OID names a different object. A v2c trap that carries no `snmpTrapOID.0` at all now has no trap OID (`null`) instead of an empty string. Upgrade the poller.
- **A database error during network-map derivation no longer empties the map.** If the node list could not be read, the derivation treated the fleet as empty, derived no links, and then deleted every stored link older than fifteen minutes — which, with inputs that change hourly, was all of them, until the next observation changed. If the link overrides could not be read, it ran without them, so a pinned link was deleted and a hidden one was put back on the map. A derivation that cannot read every input now changes nothing and tries again five minutes later.
- **A database error no longer clears a poller-pool coverage alert, or makes a pool action guess.** The coverage check counted nodes per pool while reading the folder tree and the node list, and treated a read error as "no inheritance" or "no nodes": a pool whose nodes all inherit it looked empty and its open "no live poller" alert was resolved, and after a restart one failed read resolved every restored coverage alert. A failed read now changes nothing until a read succeeds. For the same reason, **Poll now** (single and bulk), **covering a pool from another** and **moving a poller to another pool** now answer `500` when the folder tree or node list cannot be read, instead of sending a folder-inheriting node's jobs to the default pool, pinning nodes from other pools, or letting a pool's last poller leave while its nodes still depend on it.
- **A Meraki response whose next-page link repeats a page no longer makes the same devices count many times.** Paging followed a `Link: rel=next` back to a page it had already fetched, up to the 50-page cap, adding the same items each time. It now stops at the first repeated page.
- **The hourly row-name walk no longer makes the next check on the same device get skipped.** The walk ran for up to 20 seconds after a table poll, on top of that poll's time budget, and a check waiting for the device only waits for the budget. The walk now ends within the table poll's budget; a device whose table poll uses all of it gets its row names on a later attempt.
- **`POST /api/v1/discovery/import` refuses more than 1,024 nodes in one request** (`400 too_many_nodes`), the most one sweep can target. It was the one bulk node write with no limit, and a large body held a database connection and a transaction open while it inserted row by row. The WebUI never sends more than one sweep's results.
- **A stored credential damaged in the database is reported as unreadable instead of crashing the check that reads it.** A sealed secret whose stored nonce had the wrong length made decryption panic rather than fail, so the credential health check and `yagra-core verify-secrets` — which exist to name such a row after a restore — stopped on it, and so did building poll jobs for a node bound to it. It now counts as a credential that cannot be opened.
- **`snmp_walk_complete` now reads 0 when an interface poll's name walk is cut short.** An interface poll walks the numbers first and then the interface names and descriptions with what is left of its time budget; only a cut in the first walk used to count. On a large, slow device the second is the one that runs out, leaving new ports unnamed and changed descriptions unrecorded while the gauge read 1. ⚠️ Devices whose name walk runs out on every poll will start raising the walk-incomplete warning.
- **Charting `ifSpeed`, `ifType` or a duplex column as a metric no longer empties that interface column.** When a collection item declared one of those columns as a metric, the poller published the metric and skipped reading it for the interface list, so new ports got no speed, type, duplex or medium. No built-in item does this; an item added in the collection editor could. Both now happen, as they already did for `ifHighSpeed`.
...
- **One flow exporter can no longer push every other exporter's templates out of the cache.** Templates were kept in one arrival order for all exporters, 8,192 in total, and one NetFlow v9 datagram could define more than 8,000 of them — so two large datagrams from one source stopped flow decoding for every other device until each re-sent its templates, which exporters do every 1 to 30 minutes. One datagram may now add at most 128 new templates, one exporter address holds at most 512 and makes room from its own oldest, and when the cache is full the exporter holding the most gives up its oldest. Re-sending a template already held costs nothing. Counted as `yagra_flow_templates_dropped_total{reason="datagram_cap"|"exporter_quota"|"global_cap"}`. A flood with forged source addresses can still fill the cache; restrict who can reach the collector port. Upgrade the poller.
- **A folder-scoped account can no longer change or delete nodes outside its folders by id.** Eleven per-node writes checked the permission and not the scope, so an account limited to some folders could — through the API, not the WebUI — delete any node; change its name, profile, credential, labels or pool; move it; set its dependency parent; add a collection item; or add, change or remove its URL or DNS check, and learn from the status code whether an id existed. They now answer `404 node_not_found` for a node outside the caller's folders, exactly as the reads do. Moving a node also refuses a destination folder the caller cannot see (`404 group_not_found`, or `403 out_of_scope` for "no folder"), and a dependency parent outside the caller's folders is refused as `400 parent_not_found`. The routes: `DELETE /api/v1/nodes/{node_id}`, `PUT …/bindings`, `…/group`, `…/parent`, `…/placement`, `…/pool`, `POST …/collection`, and `PUT`/`DELETE …/url-check` and `…/dns-check`. Accounts with no folder restriction are unaffected. Only the core needs upgrading.
...
- **A Huawei chassis part with no temperature sensor no longer reads 2147483647 °C.** Some Huawei devices — measured on an AC6508 wireless controller — answer the entity temperature column with 2147483647 for every chassis part that has no sensor, and Yagra stored it as a temperature: the node's temperature card read 2147483647 °C and the built-in rule opened a critical alert for each such part. That value is now recognised as "no reading" for this column. It is not stored, charted or checked against any rule, and a per-part alert it raised closes as an ordinary recovery once the device has answered it for the rule's dwell (two polls for the built-in rule), so expect one recovery notification per affected part after the upgrade. A part that merely stops being reported keeps its alert open, so a collection fault stays visible. A reading of 0 °C is still a reading, and points already stored remain in chart history. Only the core needs upgrading.
...
- **Saving your own WebUI preferences no longer writes an audit row.** `PUT /api/v1/preferences` changes nothing but your own screen, and it is now sent each time you open or close a folder, so those rows would have filled an audit log that is kept forever. Sign-in and every change to monitoring or to what other people see are audited as before. The preferences document may now be up to 32 KiB (was 16 KiB).
...
- **A node's Interfaces, Collection, Events and Flow filters survive a reload, and stay on as you move between nodes.** They were held by the tab alone, so a reload cleared them, and so did selecting another node in All nodes. They are now in the URL under the tab's name (`?interfaces.oper=`, `?events.message=`, `?flow.port=`), which also keeps them apart from the inventory tree's own filters on the same page — the tree's `kind` and the Events tab's `kind` no longer share a name. Picking another node in the tree keeps the tab's filter in force, and the tab's **Clear all filters** removes it. The interface whose charts are open is still not remembered: an interface index names a different port on the next device.
- **Closing a popover, a dialog or the interface chart dock with Escape no longer also clears the node selected in All nodes.** The page and the surface above it both listen for Escape, and which heard it first depended on which had registered last. Once a filter had been changed, the page could hear the press after the popover had already closed, find nothing open, and clear the selection as well. A surface that closes on Escape now marks the key press as handled, and the page leaves a handled press alone.
- **Filters on Users, Audit, Saved findings, Device profiles, Metric sets, Troubleshoot ▸ Runs, the MIB repository and Discovery survive a reload.** Each of these screens kept its filters out of the URL, so a reload showed the unfiltered list again — and for Audit and Saved findings, which filter on the server, re-issued the unfiltered query. They are now in the URL like every other list's. Saved findings also writes its node or folder scope as `?node_id=` / `?group_id=`, as Alert history already did, and opens with the picker showing that scope. The MIB repository's search is `?q=`. Discovery's two tables are `?candidates.…` and `?endpoints.…`; the seen-endpoints table still opens on unmonitored endpoints, and choosing all of them is kept too.
- **A Troubleshoot report's chips and sort, and the sort order on API tokens and Credentials, survive a reload.** Choosing "Level shift" or sorting a report by node, or sorting a table by a column, lasted until the next reload. They are now in the URL (`?filter=`, `?sort=`, `?dir=`, and `?group_by=` / `?show=` on the two reports that have those), and running a report again keeps the chips as they were instead of resetting them.
- **A Huawei CloudEngine S5735-L stack shows every member's serial number.** On these switches (YunShan OS / V600) each member's main board is named after its model and member number, such as `S5735-L8P2T4X-A-V2 1`, rather than `MPU Board N`, so the Overview kept showing only the first member's serial. A main board is now also recognised by where it sits: a board directly in the stack chassis's `MPU slot N`, named with the same number, is member N's. A two-member S5735-L stack shows both serials. The new value appears at the next hourly identity read, or at once with Poll now. On Huawei devices, the poller now also reads the ENTITY-MIB column that says which component each row sits in.
- **A folder closed while the inventory tree is filtered stays closed after the filter is cleared.** Closing a folder with a search term, or a state, kind or pool filter, on worked on screen, but clearing the filter opened it again. The press is now saved like any other. While a filter is on the tree still starts with every folder open, so a folder closed earlier cannot hide a match; pressing it there closes it, and it stays closed after the filter is cleared. Reloading the page with the same search keeps it closed as well, in the same browser tab; opening the same link in a new tab or another browser starts with every folder open, as any new search does.
...
- **Poll now works on a selection, and is reachable from the inventory tree.** It lived only on a node's detail header, which is the wrong place for it: it is what an operator presses right after editing something in the tree, and after a bulk edit it was the only way to check the change took — one node at a time. The tree's right-click menu now carries it, acting on the whole selection when the clicked row is part of it, and the selection bar's **More…** menu offers it too. What was dispatched is reported below the tree; a shortfall stays on screen. New endpoint `POST /api/v1/nodes/poll` (`{node_ids}` → `202 {requested, dispatched, jobs}`), scoped to the caller's folders and capped at 1000 nodes.
- **Polling a node no longer makes the whole fleet re-resolve its poll configuration.** `POST /api/v1/nodes/{node_id}/poll` counted as a configuration change, so every press of Poll now invalidated the cached monitoring configuration and rebuilt it for every node. A manual poll dispatches a node's existing checks and changes no configuration, so it no longer bumps that signal. Behaviour of the poll itself is unchanged.
- **Maintenance windows and mutes can be opened over every selected node at once.** "This dozen, tonight" rarely follows a folder boundary, so the folder-scoped window could not express it and the per-node one meant a right-click each — and the presets in the tree's context menu silenced only the clicked node even while the same menu offered "Move 20 selected…", so an operator silencing a dozen devices covered one and found out by being paged during the work. The presets now act on the whole selection when the clicked row is part of it and say so in their heading, and the selection bar's **More…** menu carries both. One window (or mute) is created per node. New endpoints `POST /api/v1/maintenance-windows/bulk` and `POST /api/v1/mutes/bulk`, each scoped to the caller's folders, capped at 1000 nodes and answering `{requested, created}`. The release panel still acts on the row it was opened from. The selection bar now appears for an operator who holds only the suppression permissions.
- **The poller pool can be set on every selected node at once.** Re-homing a site onto different pollers rarely follows folder boundaries, so neither the folder-wide pool nor the per-node one served it — and the pool chips in the tree's context menu wrote only the right-clicked node even while the same menu offered "Move 20 selected…". The chips now act on the whole selection when the clicked row is part of it, and say so in their heading; the selection bar gains a **More…** menu carrying the same action. A chip is shown as already-selected only when every node in the selection agrees. New endpoint `POST /api/v1/nodes/pool` (`{node_ids, pool}` → `{requested, applied}`), scoped to the caller's folders and capped at 1000 nodes per request; an omitted or empty `pool` clears the nodes back to inherited.
- **A context-menu item that acts on one node now says which one, while several are selected.** Right-clicking a selected row opens a menu offering "Move 20 selected…" alongside "Edit node…" and "Pin", and those two act on the clicked row only — they have no bulk form, because the edit dialog shows one node's whole binding and a pin is the account's own navigation. They now read `Edit "name"…` and `Pin "name"` whenever a selection is in play, matching what the move items have always done. With nothing else selected the labels are unchanged.
- **Tagging several nodes at once is offered on the selection bar.** It was reachable only by right-clicking a selected row, so an operator working from the bar had no way to discover it.
- **Dragging several selected nodes between two rows now puts them there.** Selecting nodes with Ctrl/Shift and dropping them between two rows appended them to the end of the destination folder instead, so the same gesture answered differently at one node and at several — reordering a group of nodes within one folder was not possible at all. `POST /api/v1/nodes/move` takes optional `before`/`after` fields naming the sibling node to land next to, and writes the whole batch in one statement, keeping the order the nodes were selected in. Omitting both appends as before, so an older WebUI is unaffected. The WebUI no longer calls `PUT /api/v1/nodes/{node_id}/placement`; that endpoint is unchanged and still served for API clients.
...
- **Find nodes that are one device added more than once, and delete the copies.** A new Nodes ▸ Duplicates screen lists device nodes that look like the same device registered twice or more — at the same address, or at two different ones, such as a router added once by its loopback and again by a LAN address. Nodes that share an address, a serial number, a MAC address in a router's ARP cache or an LLDP chassis ID, or that each list the other's address among their own, are grouped as a likely duplicate. Nodes that share only two weaker signs — the same name, the same CDP device ID, or one listing the other's address — are grouped as to check, and so is any group whose serial numbers or models disagree. A value that more than 8 nodes share is not used, and the screen lists those values. One node per group is marked as the suggested keep (the most depended-on, then the oldest), but nothing is selected for you, and a delete that would remove every node of a group is refused. Deleting uses the same bulk delete as All nodes. The screen needs the manage-configuration permission and shows only the nodes in your folders. The API is `GET /api/v1/nodes/duplicates`, and the MCP `get_config` tool answers the same with `kind=duplicate_nodes`.
...
- **A node's Overview shows its serial number.** A new Serial number row under Model shows the serial the device reports for its chassis in ENTITY-MIB, which the poller reads once an hour together with the OS version. A stack or an SVL pair lists every member, in order, separated by commas. A Meraki device shows the serial it was imported with. A Juniper device is read from Juniper's own MIB first: a Virtual Chassis lists every member by member ID, and any other Junos box shows its box serial (`jnxBoxSerialNo`). A device that keeps a serial in neither place shows "—". A device whose ENTITY-MIB has no class column at all and reports exactly one serial shows that one. The value appears within an hour of the poller being upgraded. `GET /api/v1/nodes/{node_id}` and the MCP `get_node_status` tool return it as `serial_number`.

### Improvements

- **Poll now also reads what is otherwise read only once an hour.** Pressing Poll now on a node — or calling the MCP `poll_now` tool — polled everything except two things the poller reads on its own hourly schedule: the device's identity (its OS version and serial number) and the names of its memory-pool, CPU and sensor rows. A node showing "—" for its OS version stayed that way for up to an hour. Both are now read on the same poll, and the node page reads again 30 seconds after the press, so they appear without reopening it. The next hourly read is counted from that poll. A poller older than core ignores the request and keeps its hourly schedule.
- **A device that answers none of the scalar values in its profile still gets its identity read.** A device whose SNMP agent answers but implements none of the profile's scalar OIDs (sysUpTime and the like) used to be treated like one that did not answer at all, so its vendor, model, OS version and serial number were never read. They are now read on the hourly identity probe. `snmp_up` still reads 0 for such a device, and the SNMP-down rule still fires on it.
...
- **Memory, CPU and temperature are judged and shown per row — per memory pool, per CPU, per sensor.** A switch reports memory separately for each pool (`Processor`, `I/O`, …) and a chassis a CPU and a temperature per board. Until now an alert fired on the worst of them without saying which, and Device health showed a number that could come from a different one: on a Catalyst 2960S an alert said memory was above 80% (83.9) while the card said 56%, because the alert was about the I/O pool and the card showed the Processor pool. Now each row is its own check with its own alert, named after the row — `cisco_mem_used_pct on I/O above 80 (was 83.9)` — and Device health lists every row by name under its card. The memory card's headline and chart follow the fullest pool. The poller reads the row names from the device once an hour: a Cisco memory pool's own name, a Huawei board's ENTITY-MIB name, and so on. A row whose value is zero (a port on a Huawei stack has no memory of its own) is not listed.
  - **Threshold rules can name the rows they apply to.** Alerts ▸ Metric alert rules has a **Row name** field (`I/O`, `MPU Board *`; `*` matches anything, case is ignored). At the same scope, a rule with a row name wins over the rule for every row for the rows it names, so one pool can be loosened or tightened beside the rule for all of them. A new default rule sets Cisco IOS's `I/O` pool to warn at 90% and go critical at 95%: that pool holds packet buffers the switch pre-allocates, and on a healthy switch it sat at 84% for a week.
  - **API:** `GET /api/v1/nodes/{node_id}/metrics/{metric}?rows=true` returns `rows` (each row's key, name and latest value), and the range read takes `row` to chart one row. Alerts, alert history and the MCP alert tools carry `row` and `row_name`; threshold rules and the configuration bundle carry `row_match`. MCP `query_metrics` returns `rows` and takes `row`. Notification templates can use `{{ row_name }}`.
  - ⚠️ An alert that was open on one of these metrics before the upgrade is resolved on the first poll after it and, if a row is still over its bound, opens again on that row — an external tool (PagerDuty, JSM) sees one incident close and a new one open under a different dedup key. Row names appear within an hour of the poller being upgraded; until then a rule with a row name matches nothing and the rule for every row applies.
- **The Interfaces tab shows what each port is connected to.** A new Neighbors column, between Description and Oper, names the device the port sees over CDP or LLDP, with `+1` when there are more. Click the name to see every neighbor on that port — its port, capabilities and management address — without leaving the list or opening the port's charts.
  - A CDP neighbor is placed by the port's index. An LLDP neighbor is placed only when the device names the port the same way as the interface list does, so a device that reports the port differently (a Junos switch reporting it by number, for example) shows its neighbors in the Neighbors tab only.
  - The list is 120px wider, so it scrolls sideways in browser windows narrower than about 1,260px. The column is not shown on a phone.

### Improvements

- **A new installation polls every 5 minutes instead of every 30 seconds.** Thirty seconds is more than most networks need, and every poll is work for the device and for Yagra. An existing deployment keeps the interval it has stored: the default is written once, when a deployment is installed, and an upgrade does not change it.
  - ⚠️ At five minutes, a node that stops answering is reported down after about 15 minutes (the default rule waits for three missed polls). Where that is too slow, lower the interval in Settings ▸ Monitoring defaults, or on a device profile.
- **Adding a node, or changing a poll interval, takes effect straight away.** The scheduler used to pick either up on its next round, which is one poll interval — up to five minutes at the new default.

### Bug Fixes

- **Traffic charts, IN/OUT and traffic rankings no longer go blank on nodes polled every 5 minutes or less often.** A rate was always worked out over the last 5 minutes, and a window that holds fewer than two polls has no rate: on a node polled every 10 minutes the Interfaces tab drew five minutes of traffic and five minutes of nothing, and IN/OUT read "—". The window now holds at least two of the node's polls — on the Interfaces tab, a node's rate charts, the utilization heatmap, Top interfaces, fleet throughput, traffic spikes and drops, and interface utilization alerts. A deployment polling every 150 seconds or faster sees no change.
  - **API:** the `window` of `GET /api/v1/metrics/interface-delta` (and `window_secs` of MCP `top_interfaces`) is widened to at least twice the slowest poll interval in the fleet, because a shorter window has nothing to compare.
...
- **A slow SNMP device now gets up to half its poll interval to read its interface table.** An interface poll is two reads — one for traffic counters, port status and the like, one for port names — and each used to get 16 seconds whatever the poll interval, checked only between columns. A switch polled every minute that needed 20 seconds therefore lost several columns on half its polls. Now one time budget covers the whole interface poll: half the poll interval, at least 16 and at most 150 seconds (30 seconds at a one-minute interval). The counters read gets the first three quarters of it and the names read the rest, and a read that reaches its deadline stops before its next request even part-way through a column, keeping the rows it already has. ⚠️ At an interval of 32 seconds or less, and for **Poll now**, the counters read now gets 12 seconds instead of 16. A poll waiting for an earlier poll of the same device now waits up to that device's budget plus 15 seconds when the interval is over 90 seconds (it was capped at 60). A poll that read every column is no longer reported as incomplete (`snmp_walk_complete` = 0) just because its last page arrived after the deadline. For a device too slow for any budget, a longer poll interval is still the direct fix — at 300 seconds it gets 150.
- **A slow SNMP device no longer loses a whole column of data to one late reply.** The poller reads each column of an SNMP table (traffic counters, port status, CPU and so on) a page at a time. One page answered later than the two-second timeout used to throw away the whole column — including the pages already read — and two such columns in a row made the poller treat a device that was answering as silent and stop reading it for that poll. Now a page that goes unanswered, on a device that has already replied during that poll, is asked once more, and rows already read are kept even if the column then fails. A device that does not answer at all is still given up on as quickly as before. For operators reading the poller's own metrics: `yagra_snmp_page_retries_total{result="recovered"|"failed"}` counts the re-sent pages. SNMPv3 devices do not retry yet.
...
- **A site's subnets come across from NetBox, and discovery can be aimed at them.** A sync now also
  ...
  in a range. A token that may not read `ipam.view_prefix` **changes nothing**: the folder tree
  still syncs, the stored prefixes are left alone, and the sync result says the listing was
  refused. Verified against NetBox 4.6, where a prefix's attachment is `scope_type`/`scope_id`;
  the 3.x `site` field is read too but untested.
...
- **Settings ▸ Pollers ▸ "Accept remote pollers" now works. It had never once succeeded since it
  ...
  The change now recreates the web edge too. (6) With that fixed, core connected to the encrypted
  bus with no credentials at all and was refused: the NATS client library does not read the
  username and password from the connection URL, and core was handing them over that way — the
  poller had always supplied them correctly, so a working implementation sat beside the broken one.
