...
- **All nodes left open no longer goes stale.** The folder health bars, the node total in the header and the "N need attention" count were read when the page opened and after a change you made, and never again — while the live stream went on recolouring the dots beneath them, so a wall display showed red dots under a green bar. They are now re-read every minute. When the live stream reports that it fell behind — or drops and reconnects, which misses whatever happened in between — the page now re-reads everything instead of ignoring the notice; until now a node whose recovery was in the missed part stayed red for as long as the tab was open, because the live state overrules every later read. Also on this screen: a search that fails now says so with a Retry button instead of drawing an empty tree, which under **Needs attention** read as "nothing needs attention"; the header no longer shows the handful of nodes it happened to have loaded as the fleet total when the summary cannot be read; and deleting a folder before its node count has arrived says its member nodes are still being counted instead of "0 member nodes".
