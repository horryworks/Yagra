...
- **A node dragged down onto a folder now goes into that folder.** In the inventory tree, the row showing where a drag will land takes up a row of space, and "into this folder" draws no such row. Coming down from a node onto the folder beneath it, the row disappeared at the moment the folder became the target, the folder slid up by one row, and the pointer — which had not moved — was over the next row down. Letting go there filed the node in the parent folder, a moment after the folder had been outlined as the destination. A drop now goes where the screen last said it would, which is how dropping on the insertion row itself has always worked.
- **Five smaller fixes in the inventory tree.** A node released on a folder's "Loading nodes…" or "could not load" row now goes into that folder; those rows are drawn inside the folder and read as part of it, but refused the drop, so the node went nowhere while the folder above stayed outlined. A drag that ends anywhere — over the detail pane, with Escape, or after the row you grabbed has scrolled out of the list — no longer leaves rows dimmed and an empty insertion row wedged into the tree. Opening a row's menu from the keyboard (the Menu key or Shift+F10) when that row is scrolled out of view now opens it beside the row; it used to open at the corner of the tree pane. The row buttons (add, edit, delete, open) are now always visible on a touch screen, where there is no hover to reveal them and, on an iPhone or iPad, no long-press menu to fall back on. And when **With nodes** hides every folder because none holds a node yet, the tree says so instead of drawing a lone "Ungrouped 0".
