...
async fn main() -> anyhow::Result<()> {
    ...
    let transport: Arc<dyn sekisho::Transport> = Arc::new(sekisho::SurgePingTransport::new()?);
    ...
    let queue =
        std::env::var("YAGRA_POLLER_QUEUE").unwrap_or_else(|_| hikyaku::POLLER_QUEUE.to_owned());
    let jobs = Box::pin(bus.subscribe_jobs(&queue).await?);
    tracing::info!(%queue, "Yagra-poller (banshu) consuming jobs");
...
async fn connect_bus(url: &str) -> anyhow::Result<hikyaku::NatsBus> {
    const MAX_ATTEMPTS: u32 = 30;
    let mut attempt = 0;
    loop {
        match hikyaku::NatsBus::connect(url).await {
