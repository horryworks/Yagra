...
pub(crate) const FOLDED_READS: &[FoldedRead] = &[
    ...
    //
    // The configuration-read family, 33 routes behind one `kind`. This is the block that proves the
    // module doc's point about permission: it spans `ManageConfig` ×13, `View` ×14,
