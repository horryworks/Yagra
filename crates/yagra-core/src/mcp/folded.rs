...
//!
//! The measured spread across the routes folded here is `View` ×44, `ManageConfig` ×13,
...
//! nothing could check it; that reasoning holds there and not here — over these 70 rows the
...
pub(crate) const FOLDED_READS: &[FoldedRead] = &[
    ...
    //
    // The configuration-read family, 32 routes behind one `kind`. This is the block that proves the
    // module doc's point about permission: it spans `ManageConfig` ×12, `View` ×14,
    ...
    FoldedRead {
        ...
        arg: "netbox_servers",
        method: "GET",
        path: "/api/v1/netbox/servers",
        // `View`, matching `list_netbox_servers`'s `RequireView` — the guard walks the document's
        // operationId to that signature, so this cannot be asserted here and be wrong there.
        perm: Some(Permission::View),
        inventory_ids_ok: None,
        opaque_ok: None,
        lowered_to: None,
