...
// Self-import so the sweep loop below keeps the `scheduler::` paths it was written with — it lived
// in `main.rs` until ADR-083, where that prefix was the only way to name this module.
...
mod tests {
    ...
    // ── Sweep pool grouping and the spec cache (moved from main.rs, ADR-083) ──
