...
mod tests {
    ...
    /// The text between a handler's parentheses — its argument list, which the guards below read to
    /// see which extractors it takes.
    ///
    /// 🚨 **Matched by counting parentheses, not by finding the first `)`.** The first-`)` version
    /// truncated the argument list at any `)` inside it — including one in a `// comment` on an
    /// argument, which is exactly where a reason for an extractor belongs. The failure was at least
    /// loud (a truncated list contains fewer extractor names, so the guard reports the handler as
    /// missing one) but it accused the wrong thing: the handler was correct and the reader was not.
    /// Found when a comment reading `(ADR-125)` cost two handlers their `Scoped`.
