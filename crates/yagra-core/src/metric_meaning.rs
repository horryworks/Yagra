...
pub const METRIC_MEANINGS: [(&str, &str, MetricUnit); 108] = [
    ...
    ("cisco_cemp_mem_used_pct", "Percentage of an enhanced memory pool in use. Computed from cempMemPoolUsed and cempMemPoolFree — Cisco reports the two halves and no total, so this figure exists in no series and cannot be charted. One value per pool; the worst pool is the one a rule fires on.", MetricUnit::Symbol("%")),
