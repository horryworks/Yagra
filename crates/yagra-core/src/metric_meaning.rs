...
pub const METRIC_MEANINGS: [(&str, &str); 107] = [
    ...
    ("cisco_cemp_mem_used_pct", "Percentage of an enhanced memory pool in use. Computed from cempMemPoolUsed and cempMemPoolFree — Cisco reports the two halves and no total, so this figure exists in no series and cannot be charted. One value per pool; the worst pool is the one a rule fires on."),
    ...
    ("cisco_cpu_mem_used_pct", "Percentage of a per-CPU memory pool in use, computed from cpmCPUMemoryUsed and cpmCPUMemoryFree. Not collected — see cisco_cemp_mem_used_pct for why."),
    ...
    ("cisco_mem_used_pct", "Percentage of a memory pool in use, computed from ciscoMemoryPoolUsed and ciscoMemoryPoolFree. The older pool table; newer IOS-XE and IOS-XR answer on cisco_cemp_mem_used_pct instead. Not collected."),
    ...
    ("hr_storage_used_pct", "Percentage of a storage area in use, computed from hrStorageUsed and hrStorageSize. ⚠️ hrStorageTable holds filesystems, physical memory and swap together, so a node has several of these and the rule fires on the worst. Both halves are read from the same row — dividing one row by another would report the wrong disk. Not collected, so it has no chart."),
    ...
    ("huawei_mem_used_pct", "Percentage of installed memory in use, computed from the total and free readings in HUAWEI-MEMORY-MIB. Not collected. On a device that also reports huawei_mem_usage the two should agree; that one is the vendor’s own per-board figure, this one is whole-box."),
    ...
    ("poe_power_used_pct", "Percentage of a PoE group’s power budget drawn by attached devices, computed from pethMainPseConsumptionPower over pethMainPsePower. Not collected. No default rule ships for it: what counts as over-subscribed is a site decision, not a device fault."),
    ...
    ("ucd_cpu_used_pct", "CPU in use, in percent — 100 minus ssCpuIdle. Not collected; it exists so a rule can read the usual way round (alert *above* a bound) instead of below one."),
    ...
    ("ucd_mem_used_pct", "Percentage of physical memory in use, computed from memTotalReal and memAvailReal. ⚠️ On Linux memAvailReal excludes the page cache, so this reads higher than free(1)’s \"available\" column. Not collected."),
    ...
    ("ucd_swap_used_pct", "Percentage of swap in use, computed from memTotalSwap and memAvailSwap. A host that has started consuming swap is usually already in trouble, so a low bound is reasonable here. Not collected."),
