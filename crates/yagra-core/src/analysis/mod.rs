...
// The one accessor every source-text check reads this module through, and the checks
// themselves (ADR-089). Both are `#[cfg(test)]`, which is how `source`'s exclusion derives
// them — a guard file inside the text it greps matches its own needles.
#[cfg(test)]
mod guards;
#[cfg(test)]
mod source;
