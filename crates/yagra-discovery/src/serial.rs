...
//! L18004ds011, a two-member S5735-L-V2 on YunShan OS: each member's main board is called after its
...
mod tests {
    ...
    /// box — TDC1004CO01 on 2026-09-15 (no `entPhysicalContainedIn`) and L18004ds011 on 2026-09-16;
    ...
    const HUAWEI_RECORDINGS: &[HuaweiRecording] = &[
        ...
        HuaweiRecording {
            name:
                "PoC L18004ds011, a two-member CloudEngine S5735-L-V2 stack, boards named by model",
            rows: &[
            ...
            chassis_rule: Some("QU23C6032037"),
            shown: Some("QU23C6032037, QU23C6032056"),
            by_boards: true,
        },
        HuaweiRecording {
    ...
    /// A CloudEngine stack of one member as L18004ds011 nests it — a chassis with no serial of its
    ...
    fn without_containment_a_cloudengine_stack_keeps_the_chassis_rule() {
        ...
        assert!(recording.name.contains("L18004ds011"));
        let maps = HuaweiMaps::flat(recording.rows);
        assert_eq!(maps.boards(), None);
        assert_eq!(maps.chassis_rule().as_deref(), Some("QU23C6032037"));
    }

    /// The shape of the PoC's L18004cs001, a four-member stack of two models — the row indexes and
