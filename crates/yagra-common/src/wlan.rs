...
///
/// Serialized as lower-case, colon-separated hex (`54:f6:e2:0a:02:80`), which is also how Meraki's
...
impl ApMac {
    ...
    /// From six SNMP sub-identifiers — how a MAC-indexed table spells its row (`…84.246.226.10.2.128`).
...
mod tests {
    ...
    fn a_mac_reads_the_same_from_every_spelling() {
        let from_index = ApMac::from_subids(&[84, 246, 226, 10, 2, 128]).expect("six bytes");
        assert_eq!(from_index.to_string(), "54:f6:e2:0a:02:80");
        assert_eq!(ApMac::parse("54:F6:E2:0A:02:80"), Some(from_index));
        assert_eq!(ApMac::parse("54-f6-e2-0a-02-80"), Some(from_index));
        assert_eq!(
            ap_id(from_index),
            ap_id(ApMac::parse("54:f6:e2:0a:02:80").unwrap())
        );
    }
    ...
    fn a_mac_that_is_not_six_bytes_is_refused() {
        ...
        assert_eq!(ApMac::parse("54:f6:e2:0a:02"), None);
        assert_eq!(ApMac::parse("54:f6:e2:0a:02:8"), None);
        assert_eq!(ApMac::parse("zz:f6:e2:0a:02:80"), None);
    }
    ...
    fn an_ap_id_is_pinned_to_its_mac() {
        let mac = ApMac::new([0x54, 0xf6, 0xe2, 0x0a, 0x02, 0x80]);
        assert_eq!(
            ap_id(mac).to_string(),
            Uuid::new_v5(&WLAN_AP_NS, &[0x54, 0xf6, 0xe2, 0x0a, 0x02, 0x80]).to_string()
        );
        assert_ne!(
            ap_id(mac),
            ap_id(ApMac::new([0x54, 0xf6, 0xe2, 0x0a, 0x02, 0xa0]))
        );
        assert_eq!(
    ...
    fn a_mac_serializes_as_a_string_and_back() {
        let mac = ApMac::new([0x60, 0x10, 0x9e, 0x0a, 0x03, 0xa0]);
        let json = serde_json::to_string(&mac).unwrap();
        assert_eq!(json, "\"60:10:9e:0a:03:a0\"");
