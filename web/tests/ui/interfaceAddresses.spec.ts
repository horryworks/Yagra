...
const SVI_RECORDED: Address[] = [
  { ip: '10.204.29.254', prefix_len: 24 },
  ...[1, 2, 3, 4, 5, 6, 7, 8].map((n) => ({ ip: `10.221.${n}.254`, prefix_len: 24 })),
  { ip: '10.225.171.254', prefix_len: 24 },
  { ip: '10.225.187.254', prefix_len: 22 },
];
...
function interfaceRows(svi: Address[] = SVI_SHORT): Json {
  ...
  return [
    ...
    port({
      ...
      addresses: [{ ip: '10.203.250.42', prefix_len: 30 }],
    }),
...
test('each port shows its first address as ip/prefix, +N for the rest, and a dash for none', async ({
  ...
  await expect(p2p.locator('.nd-if-addr-first')).toHaveText('10.203.250.42/30');
...
test('+N opens every address beside the cell, and never the dock', async ({ page }) => {
  ...
  await pop.getByText('10.221.1.254/24').click();
  await expect(pop).toBeVisible();
  await expect(page.locator('.nd-if-dock')).toHaveCount(0);
...
test('Escape closes the address popover before the dock, and the dock lists every address', async ({
  ...
  await expect(page.locator('.nd-if-dock-addrs')).toContainText(
    '10.204.29.254/24, 10.221.1.254/24, fec0::a:0:0:4',
  );
...
test('the address filter finds a secondary hidden behind +N', async ({ page }) => {
  await openTab(page);
  // `10.221.` is only in Vlanif100's SECOND address — the one the cell does not show.
  await page.goto(`/nodes/${NODE_ID}?tab=interfaces&interfaces.addresses=10.221.`);
...
test.describe('on a phone', () => {
  ...
  test('the column is not drawn and the dock shows every address, unclipped', async ({ page }) => {
    ...
    await expect(tile).toContainText('10.204.29.254/24, 10.221.1.254/24');
    await expect(tile).toContainText('10.225.187.254/22');
