...
function SeenOnNetworkCard({
  ...
  // Per row: a Detect in flight, or what it found (ADR-179 増分 2). Screen state only, like a range
  // scan's results — a reload forgets it.
  const [detect, setDetect] = useState<Record<string, 'running' | DetectResult>>({});
  // Whether this card is still mounted: a probe's poll loop outlives the click that started it and
  // must not write state into a card the operator has navigated away from.
  ...
  // Detect: a one-address scan through the observer's pool, polled until it finishes, then the two
  // dropdowns are filled from what answered. Nothing is imported — the operator reads the result and
  // presses Monitor. One read outstanding at a time, like the Scan tab's own poll.
  const detectOne = async (e: DiscoveredEndpoint) => {
    setDetect((cur) => ({ ...cur, [e.id]: 'running' }));
    setError(null);
    const finish = (r: DetectResult) => {
      if (!alive.current) return;
      setDetect((cur) => ({ ...cur, [e.id]: r }));
      if (r.kind === 'found') {
        setRows((cur) => ({
          ...cur,
          [e.id]: detectedSelection(
            r,
            profiles.map((p) => p.id),
            creds.map((c) => c.id),
          ),
    ...
    try {
      const { scan_id } = await api.probeDiscoveredEndpoint(e.id, probeCredIds);
      let failures = 0;
      // Bounded: a probe no poller ever takes (a pool whose pollers are all gone) must not spin
      // forever. Five minutes covers a long credential list at the sweep's 2 s spacing.
      for (let i = 0; i < 150 && alive.current; i++) {
        await new Promise((res) => setTimeout(res, POLL_INTERVAL_MS));
        if (!alive.current) return;
        try {
          const r = detectResultOf(await api.getDiscoveryScan(scan_id), e.ip);
          failures = 0;
  ...
  const detectLine = (r: 'running' | DetectResult | undefined): string | null => {
  ...
  const promote = (e: DiscoveredEndpoint) => {
    ...
    api
      .importDiscoveredEndpoint(e.id, {
        ...
        name: importNameAfterDetect(e, result),
        profile_id: r.profile_id || undefined,
        credential_id: r.credential_id || undefined,
