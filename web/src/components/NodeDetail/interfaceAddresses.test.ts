...
describe('formatAddress', () => {
  it('spells an address as ip/prefix', () => {
    ...
    expect(formatAddress({ ip: '10.103.250.42', prefix_len: 30 })).toBe('10.103.250.42/30');
...
describe('addressCellText', () => {
  it('shows the first address and counts the rest, keeping every one for the popover', () => {
    ...
    const cell = addressCellText([
      ...
      { ip: '10.121.2.254', prefix_len: 24 },
      { ip: 'fec0::a:0:0:4', prefix_len: null },
    ]);
    expect(cell).toEqual({
      first: '10.104.29.254/24',
      more: 3,
      all: ['10.104.29.254/24', '10.121.1.254/24', '10.121.2.254/24', 'fec0::a:0:0:4'],
    });
  });

  it('needs no +N for a port with exactly one address', () => {
    expect(addressCellText([{ ip: '10.103.250.42', prefix_len: 30 }])).toEqual({
      first: '10.103.250.42/30',
      more: 0,
      all: ['10.103.250.42/30'],
    });
  });
