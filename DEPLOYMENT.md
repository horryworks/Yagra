...
| `YAGRA_MAX_CONCURRENT_POLLS` | `256` | Max concurrent in-flight probes. A bound on what is *in flight*, not a rate — the polls/s it yields is this number divided by how long one probe takes. Read `yagra_poll_cycles_missed_total` before changing it: that counter, not CPU, is what says a poller is behind. The same budget also caps concurrent SNMP table walks, so lower it for a small site |
