...
| `YAGRA_PULL_POLICY` | `always` | Whether Compose re-pulls the three images on every start. The default is unchanged, so an existing deployment does not move. Set it to `missing` on a host whose images did not come from a registry — a relocation loads the three images onto the new host directly, and that host must then start itself without reaching for a registry that only ever answered on the machine it came from |
| `YAGRA_BACKUP_SKIP_METRICS` | `0` | `1` leaves the VictoriaMetrics snapshot out of a backup, and records that as an omission in the manifest exactly as an unreachable store would be. Set by a relocation whose operator unticked the metrics; the upgrade path never sets it |
